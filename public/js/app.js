...
// 沐浴完成圖示（浴缸；寶寶當日有沐浴紀錄時顯示於房況卡片）
const BATH_SVG = '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 12h16v3a3 3 0 0 1-3 3H7a3 3 0 0 1-3-3z"/><path d="M6 12V6a2 2 0 0 1 4 0"/><line x1="7" y1="18.5" x2="7" y2="20.5"/><line x1="17" y1="18.5" x2="17" y2="20.5"/></svg>';
...
async function viewBabyCare() {
  ...
  const flash = t => {
    ...
    el.textContent = t;
    setTimeout(() => { const m = $('#bc-msg'); if (m) m.textContent = ''; }, 2000);
  };
  // 一鍵記錄／位置異動共用跳窗：日期（預設今日、可改）＋備註
  const quickModal = (title, hint, onSave) => openModal(title, `
    ${hint ? `<p style="font-size:.85rem;color:var(--muted);margin:0 0 8px">${hint}</p>` : ''}
    <div class="field"><label>日期</label><input type="date" id="qm-date" value="${todayStr()}"></div>
    <div class="field"><label>備註<small>（可留空）</small></label><input id="qm-note"></div>
    <div class="row mt"><button class="btn" id="qm-save">確認記錄</button><span class="error-msg" id="qm-err"></span></div>`, body => {
    body.querySelector('#qm-save').onclick = async () => {
      const date = body.querySelector('#qm-date').value;
      if (!date) { body.querySelector('#qm-err').textContent = '請選擇日期'; return; }
      try {
        await onSave(date, body.querySelector('#qm-note').value.trim());
        closeModal();
      } catch (e) { body.querySelector('#qm-err').textContent = e.message; }
    };
  });
  ...
  const renderLoc = () => {
    ...
    bar.querySelectorAll('[data-move]').forEach(btn => {
      btn.onclick = () => {
        ...
        quickModal(`位置異動：${esc(baby.name)} 移至「${LOCATION_LABEL[target]}」`,
          target === 'out' || target === 'hospital' ? '住院中／不在館內期間會自動帶入收費帳務扣抵，日期請填實際離館日。' : '',
          async (date, note) => {
            await api(`/babies/${baby.id}/location`, { method: 'PUT',
              body: { location: target, note, moved_at: `${date} ${nowHM()}` } });
            baby.location = target;
            flash(`已將 ${baby.name} 移至 ${LOCATION_LABEL[target]}`);
            $('#bc-date').value = date;
            refresh();
          });
      };
  ...
  const refresh = async () => {
    ...
    const [rows, locLogs] = await Promise.all([
      api(`/babies/${babyId}/records?date=${date}`),
      api(`/babies/${babyId}/location-logs`)
    ]);
    // 當日位置異動一併列入當日紀錄時間軸（僅顯示，編輯請走「位置異動紀錄」）
    const moveItems = locLogs.filter(l => String(l.moved_at || '').slice(0, 10) === date).map(l => ({
      t: l.moved_at, html: `
      <li>
        <div class="time">${fmtTime(l.moved_at)}　${esc(l.nurse_name || '')}</div>
        <div class="what">位置異動 <span class="badge ${LOCATION_BADGE[l.location]}" style="font-weight:400">移至 ${LOCATION_LABEL[l.location]}</span></div>
        ${l.note ? `<div class="detail">${esc(l.note)}</div>` : ''}
      </li>` }));
    const recItems = rows.map(r => ({ t: r.recorded_at, html: `
      <li>
    ...
    const items = [...recItems, ...moveItems]
      .sort((a, b) => String(b.t || '').localeCompare(String(a.t || '')));
    if (!items.length) { $('#bc-list').innerHTML = '<div class="empty">當日尚無紀錄</div>'; return; }
    $('#bc-list').innerHTML = `<ul class="timeline">${items.map(i => i.html).join('')}</ul>`;
  ...
  main().querySelectorAll('[data-quick]').forEach(btn => {
    btn.onclick = () => {
      ...
      if (!babyId) return;
      const presets = {
        wet: { record_type: 'diaper', diaper_kind: '濕', label: '濕尿布' },
        stool: { record_type: 'diaper', diaper_kind: '便', label: '大便' },
        bath: { record_type: 'bath', label: '沐浴完成' }
      };
      const { label, ...preset } = presets[btn.dataset.quick];
      quickModal(`一鍵記錄：${label}`, '', async (date, note) => {
        await api(`/babies/${babyId}/records`, { method: 'POST',
          body: { ...preset, note, recorded_at: `${date} ${nowHM()}` } });
        flash(`已記錄${label}`);
        $('#bc-date').value = date;
        refresh();
      });
  ...
  // 臍帶掉落：一次性事件，跳窗確認日期（可回填）＋備註後登記並停用按鈕
  $('#bc-cord').onclick = () => {
    const babyId = $('#bc-baby').value; if (!babyId) return;
    const baby = babyById(babyId);
    if (baby && baby.cord_off_at) return;
    quickModal(`登記臍帶掉落：${esc(baby ? baby.name : '寶寶')}`, '此紀錄只能登記一次，日期可回填實際掉落日。',
      async (date, note) => {
        const r = await api(`/babies/${babyId}/cord-off`, { method: 'POST', body: { date, note } });
  ...
  $('#bc-quickfeed').onclick = () => {
    ...
    const feeds = feedMethods();
    openModal('快速餵奶紀錄', `
      <div class="field"><label>日期</label><input type="date" id="qf-date" value="${todayStr()}"></div>
      ...
      body.querySelector('#qf-save').onclick = async () => {
        const date = body.querySelector('#qf-date').value;
        if (!date) { body.querySelector('#qf-err').textContent = '請選擇日期'; return; }
        try {
          await api(`/babies/${babyId}/records`, { method: 'POST', body: {
            ...
            recorded_at: `${date} ${nowHM()}`
          } });
          closeModal(); flash('已記錄餵奶');
          $('#bc-date').value = date;
          refresh();
...
async function viewResidents() {
  ...
  main().querySelectorAll('[data-checkout-done]').forEach(btn => {
    btn.onclick = () => openModal('退房完成確認', `
      <p style="margin:0 0 10px">確認 <b>${esc(btn.dataset.room)} 房　${esc(btn.dataset.mom)}</b> 已完成退房手續？</p>
      <p style="font-size:.85rem;color:var(--muted);margin:0 0 10px">儲存後該房將呈現空房狀態，並自動新增清潔任務：「${esc(btn.dataset.room)}房已出住」。</p>
      <div class="row mt"><button class="btn" id="co-save">儲存</button><span class="error-msg" id="co-err"></span></div>`, body => {
      body.querySelector('#co-save').onclick = async () => {
        try {
          await api(`/bookings/${btn.dataset.checkoutDone}/checkout-complete`, { method: 'POST', body: {} });
          closeModal();
          viewResidents();
...
async function wireRoomCalendar(rooms, mothers, start) {
  const cal = await api(`/room-calendar?start=${start || todayStr()}&days=30&include_daycare=1`);
  const days = [];
...
async function viewBedPlanning() {
  ...
  const [cal, rooms, mothers] = await Promise.all([
    api(`/room-calendar?start=${start}&days=60&include_daycare=1`), api('/rooms?include_daycare=1'), api('/mothers')
  ]);
  const days = [];
  for (let i = 0; i < cal.days; i++) days.push(new Date(new Date(cal.start).getTime() + i * 86400000).toISOString().slice(0, 10));
  const td = todayStr();
  ...
  const matrix = mode => cal.rooms.map(r => {
    const cells = days.map(d => {
      ...
      if (bk) {
        ...
        const movable = bk.status !== 'checked_out';
        return `<td ${movable ? `data-bk-move="${bk.id}"` : ''} title="${esc(bk.mother_name)}（${esc(bk.check_in)}~${esc(bk.check_out)}・${STATUS_LABEL[bk.status]}）${movable ? '　點擊可房號異動' : ''}" style="background:${color};padding:2px;font-size:11px;white-space:nowrap;overflow:hidden;max-width:0;${movable ? 'cursor:pointer;' : ''}${bk.status === 'checked_out' ? 'color:#8a938f;' : ''}">${isStart ? esc(bk.mother_name.slice(0, 4)) : ''}</td>`;
      }
      return mode === 'planned'
  ...
  const startM = Number(cal.start.slice(5, 7)) || 1;
  const yearOpts = [];
  for (let y = startY - 1; y <= startY + 2; y++) yearOpts.push(y);
  main().innerHTML = `
    <div class="page-title">排床</div>
    <div class="card no-print">
      <div class="row" style="gap:8px;align-items:flex-end;flex-wrap:wrap">
        <div class="field" style="max-width:150px;margin:0"><label>起始日</label><input type="date" id="bp-start" value="${esc(cal.start)}"></div>
        <div class="field" style="max-width:100px;margin:0"><label>年份</label>
          <select id="bp-year">${yearOpts.map(y => `<option ${y === startY ? 'selected' : ''}>${y}</option>`).join('')}</select></div>
        <div class="field" style="max-width:90px;margin:0"><label>月份</label>
          <select id="bp-month">${Array.from({ length: 12 }, (_, i) => i + 1).map(mo => `<option value="${mo}" ${mo === startM ? 'selected' : ''}>${mo} 月</option>`).join('')}</select></div>
    ...
    <div class="card">
      <div class="row between"><h3>${bedTab === 'planned' ? '預定床表' : '實際入住床表'}（${esc(cal.start)} 起 60 天）</h3>
  ...
  // 年份／月份選單：跳到當月 1 日的床表
  const jumpMonth = () => {
    const y = $('#bp-year').value;
    const mo = String($('#bp-month').value).padStart(2, '0');
    location.hash = `#/bed-planning?s=${y}-${mo}-01`;
    viewBedPlanning();
  };
  $('#bp-year').onchange = jumpMonth;
  $('#bp-month').onchange = jumpMonth;
  ...
  // 點床位格 → 房號異動（PUT /bookings/:id，與客戶管理／排房資料同一筆訂房，異動即同步）
...
// 任務列（清潔任務／設備報修共用）
function hkTaskRow(t, date, isAdmin) {
  return `
    <tr data-status="${t.status}">
      <td data-label="任務">${esc(t.task)}${t.note ? `<br><small style="color:#666">${esc(t.note)}</small>` : ''}</td>
      <td data-label="位置">${t.room_name ? esc(t.room_name) + ' 房' : esc(t.location || '公共區域')}${t.mother_name ? `<br><small>${esc(t.mother_name)}</small>` : ''}</td>
      ...
      <td data-label="狀態">${t.status === 'done'
        ...
        : '<span class="badge yellow">待處理</span>'}
        ${t.last_progress ? `<br><small style="color:#666">進度：${esc(t.last_progress.slice(0, 30))}</small>` : ''}</td>
      <td data-label="操作">
        <button class="btn small secondary" data-hk-prog="${t.id}">進度${t.progress_count ? ` (${t.progress_count})` : ''}</button>
        ${t.status === 'pending'
...
// 任務列操作按鈕綁定（完成／取消／刪除／進度）；refresh＝所屬頁面的重載函式
function wireHkTaskActions(tasks, refresh) {
  main().querySelectorAll('[data-hk-done]').forEach(b => b.onclick = async () => {
    await api(`/housekeeping/tasks/${b.dataset.hkDone}`, { method: 'PUT', body: { status: 'done' } }); refresh();
  });
  main().querySelectorAll('[data-hk-undo]').forEach(b => b.onclick = async () => {
    await api(`/housekeeping/tasks/${b.dataset.hkUndo}`, { method: 'PUT', body: { status: 'pending' } }); refresh();
  });
  main().querySelectorAll('[data-hk-del]').forEach(b => b.onclick = async () => {
    if (!confirm('確定刪除這筆任務？')) return;
    await api(`/housekeeping/tasks/${b.dataset.hkDel}`, { method: 'DELETE' }); refresh();
  });
  main().querySelectorAll('[data-hk-prog]').forEach(b => b.onclick = () =>
    openHkProgress(tasks.find(t => String(t.id) === b.dataset.hkProg)));
}
...
async function viewHousekeeping() {
  ...
  main().innerHTML = `
    ...
    <div class="card no-print">
      <div class="row" style="gap:10px;align-items:flex-end;flex-wrap:wrap">
        ...
        ${canAccess('#/equipment-repair') ? '<a class="btn small secondary" href="#/equipment-repair">設備報修</a>' : ''}
      </div>
    </div>
    <div class="card">
  ...
  wireHkTaskActions(data.tasks, viewHousekeeping);
}

// 設備報修（與房務清潔分開通報；共用 /api/housekeeping，僅列 kind='repair'）
async function viewEquipmentRepair() {
  const date = location.hash.split('?d=')[1] || todayStr();
  const data = await api(`/housekeeping?date=${date}`);
  const isAdmin = currentUser.role === 'admin';
  const repairTasks = data.tasks.filter(t => t.kind === 'repair');

  main().innerHTML = `
    <div class="page-title">設備報修</div>
    <div class="card no-print">
      <div class="row" style="gap:10px;align-items:flex-end;flex-wrap:wrap">
        <div class="field" style="max-width:170px;margin:0"><label>日期</label><input type="date" id="er-date" value="${esc(data.date)}"></div>
        <button class="btn small" id="er-add">新增設備報修</button>
        ${canAccess('#/housekeeping') ? '<a class="btn small secondary" href="#/housekeeping">回房務清潔</a>' : ''}
      </div>
    </div>
    <div class="card">
      <h3>維修任務（${esc(data.date)}）</h3>
      <p style="font-size:.8rem;color:var(--muted);margin:0 0 8px">設備報修與房務清潔分開通報；由「新增設備報修」選故障類別建立，處理過程可按「進度」逐筆記錄。</p>
      <div class="table-wrap"><table class="data stack">
        <thead><tr><th>故障類別</th><th>位置</th><th>排定日</th><th>狀態</th><th></th></tr></thead>
        <tbody>${repairTasks.map(t => hkTaskRow(t, data.date, isAdmin)).join('') || '<tr><td colspan="5"><div class="empty">本日尚無維修任務</div></td></tr>'}</tbody>
      </table></div>
    </div>`;

  $('#er-date').onchange = () => { location.hash = `#/equipment-repair?d=${$('#er-date').value}`; viewEquipmentRepair(); };
  $('#er-add').onclick = () => openHkTaskForm(data.residents, data.date, 'repair', viewEquipmentRepair);
  wireHkTaskActions(data.tasks, viewEquipmentRepair);
}

// 任務進度儲存（有 log）：時間序顯示歷史進度，可追加新進度（不覆蓋）
async function openHkProgress(t) {
  if (!t) return;
  const logs = await api(`/housekeeping/tasks/${t.id}/progress`);
  const where = t.room_name ? `${t.room_name} 房` : (t.location || '公共區域');
  openModal(`任務進度 — ${esc(t.task)}（${esc(where)}）`, `
    <div class="field full"><label>新增進度</label><textarea id="hkp-body" rows="2" placeholder="例如：已聯繫廠商，週五到府維修"></textarea></div>
    <div class="row" style="margin:0 0 12px"><button class="btn small" id="hkp-save">儲存進度</button><span class="error-msg" id="hkp-err"></span></div>
    ${logs.length ? `<ul class="timeline">${logs.map(l => `
      <li>
        <div class="time">${esc((l.created_at || '').slice(0, 16))}　${esc(l.staff_name || '')}</div>
        <div class="what" style="font-weight:400">${esc(l.body)}</div>
      </li>`).join('')}</ul>` : '<div class="empty">尚無進度紀錄</div>'}`, body => {
    body.querySelector('#hkp-save').onclick = async () => {
      const text = body.querySelector('#hkp-body').value.trim();
      if (!text) { body.querySelector('#hkp-err').textContent = '請輸入進度內容'; return; }
      try {
        await api(`/housekeeping/tasks/${t.id}/progress`, { method: 'POST', body: { body: text } });
        t.progress_count = (t.progress_count || 0) + 1;
        openHkProgress(t);
      } catch (e) { body.querySelector('#hkp-err').textContent = e.message; }
    };
  });
}
...
// 設備報修故障類別
const HK_REPAIR_OPTIONS = ['燈泡', '電視', '冷氣', '馬桶', '熱水', '水管', '窗簾', '垃圾桶', '房門/櫃門', '其他'];
// 公共地點（依樓層；排序：嬰兒室→住房區→會客室→大廳區）
const HK_PUBLIC_LOCATIONS = ['嬰兒室', '住房區', '會客室', '大廳區']
  .flatMap(area => ['2', '3', '4'].map(fl => `${fl}樓${area}`));
function openHkTaskForm(residents, date, mode = 'clean', onDone) {
  const isRepair = mode === 'repair';
  const cleanOpts = [...new Set([...HK_TASK_OPTIONS, ...hkTaskPresets()])];
  // 依模式提供對應選項：清潔任務 或 設備報修故障類別
  const taskOptions = isRepair
    ? HK_REPAIR_OPTIONS.map(p => p === '其他' ? '<option value="__other">其他（自行說明）</option>' : `<option value="${esc(p)}">${esc(p)}</option>`).join('')
    : cleanOpts.map(p => `<option value="${esc(p)}">${esc(p)}</option>`).join('') + '<option value="__other">其他（自行說明）</option>';
  openModal(isRepair ? '新增設備報修' : '新增清潔任務', `
    <div class="form-grid">
      <div class="field full"><label>${isRepair ? '故障類別' : '任務'}</label>
        <select id="hkt-task">${taskOptions}</select>
      </div>
      <div class="field full" id="hkt-other-wrap" style="display:none"><label>其他（請說明）</label><input id="hkt-other" placeholder="${isRepair ? '請描述故障內容' : '請描述任務內容'}"></div>
      <div class="field full"><label>對象房間／住客（可不選＝公共區域）</label>
        <select id="hkt-target">
          ${residents.map(r => `<option value="${r.room_id}|${r.mother_id}">${esc(r.room_name)} 房　${esc(r.mother_name)}</option>`).join('')}
          ${HK_PUBLIC_LOCATIONS.map(l => `<option value="loc:${esc(l)}">${esc(l)}</option>`).join('')}
          <option value="" selected>— 公共區域 / 不指定 —</option>
        </select></div>
    ...
    taskSel.onchange = () => { body.querySelector('#hkt-other-wrap').style.display = taskSel.value === '__other' ? '' : 'none'; };
    body.querySelector('#hkt-save').onclick = async () => {
      const picked = taskSel.value;
      const task = picked === '__other' ? body.querySelector('#hkt-other').value.trim() : picked;
      if (!task) { body.querySelector('#hkt-err').textContent = `選「其他」時請說明${isRepair ? '故障' : '任務'}內容`; return; }
      const target = body.querySelector('#hkt-target').value;
      const isLoc = target.startsWith('loc:');
      const [room_id, mother_id] = (isLoc ? '|' : (target || '|')).split('|');
      try {
        await api('/housekeeping/tasks', { method: 'POST', body: {
          task, kind: isRepair ? 'repair' : 'clean',
          room_id: room_id || null, mother_id: mother_id || null,
          location: isLoc ? target.slice(4) : '',
...
async function viewMeals() {
    ...
    <div class="card no-print">
      <div class="form-grid">
        ...
        <div class="field" style="display:flex;align-items:flex-end;gap:8px">
          ...
          <button class="btn secondary" id="ml-xlsx">匯出 Excel</button>
        </div>
      </div>
      <p style="font-size:.85rem;color:var(--muted);margin-top:8px">
        選擇餐點後自動儲存，並自動帶入之後每一天（同餐別），直到出住日「早餐」為止；選「未訂」只改當天。飲食注意取自住客資料，會一併帶入備餐單。
  ...
  let lastKitchen = null; // 廚房備餐單結構化資料（列印／匯出 Excel 共用）
  const refresh = async () => {
    ...
    lastKitchen = { date: data.date, grandTotal, vendors: [] };
    const sections = vendors.map(choice => {
      ...
      lastKitchen.vendors.push({ choice, total,
        tot: { breakfast: tot('breakfast'), lunch: tot('lunch'), dinner: tot('dinner') },
        rows: moms.map((x, i) => [i + 1, x.m.room_name || '', x.m.name,
          x.cnt.breakfast, x.cnt.lunch, x.cnt.dinner, x.m.diet_notes || '',
          x.m.delivery_date ? `${fmtMD(x.m.delivery_date)}${dvCode(x.m.delivery_type)}` : '',
          `${fmtMD(x.m.check_in)}午 ~ ${fmtMD(x.m.check_out)}早`,
          [x.m.meal_diet && x.m.meal_diet !== '一般' ? x.m.meal_diet : '', ...x.notes].filter(Boolean).join('、')]) });
      return `
    ...
    $('#ml-grid').querySelectorAll('[data-meal]').forEach(sel => {
      sel.onchange = async () => {
        ...
        // 選了餐點會自動往後帶到出住日早餐；選「未訂」只改當天
        await api('/meals', {
          method: 'POST',
          body: { mother_id: Number(mid), meal_date: $('#ml-date').value, meal_type: mt, choice: sel.value, note: o ? o.note : '', status: o ? o.status : 'preparing', propagate: !!sel.value }
        });
        refresh();
      };
    });
  ...
  $('#ml-print').onclick = () => {
    ...
    const sec = v => `
      ...
      <table>
        <thead><tr>${KITCHEN_COLS.map((c, i) => `<th${[3, 4, 5].includes(i) ? ' style="width:36px"' : i === 0 ? ' style="width:34px"' : ''}>${c}</th>`).join('')}</tr></thead>
        <tbody>
          ${v.rows.map(r => `<tr>${r.map((c, i) => `<td${[3, 4, 5].includes(i) ? ' style="text-align:center"' : ''}>${esc(c)}</td>`).join('')}</tr>`).join('')}
          <tr class="sum"><td colspan="3">餐點小數</td>
            <td style="text-align:center">${v.tot.breakfast}</td><td style="text-align:center">${v.tot.lunch}</td><td style="text-align:center">${v.tot.dinner}</td>
            <td colspan="4"></td></tr>
        </tbody>
      </table>`;
    const win = window.open('', '_blank');
    win.document.write(`<!DOCTYPE html><html lang="zh-Hant"><head><meta charset="UTF-8">
      <title>廚房備餐單 ${esc(lastKitchen.date)}</title>
      <style>
        @page{size:A4 landscape;margin:12mm}
        body{font-family:"Microsoft JhengHei","PingFang TC",sans-serif;color:#1c2b29;margin:20px}
        h2{margin:0 0 2px} .meta{color:#666;font-size:12px;margin-bottom:10px}
        h3{margin:16px 0 4px;font-size:15px} h3 small{font-weight:400;color:#666}
        table{border-collapse:collapse;width:100%;font-size:12.5px;table-layout:auto}
        th,td{border:1px solid #9db5b0;padding:4px 6px;text-align:left;vertical-align:top;word-break:break-all}
        th{background:#eef5f4;white-space:nowrap}
        tr.sum td{font-weight:700;background:#fff3b0}
        @media print{.noprint{display:none}}
      </style></head><body>
      <h2>${esc(center)}　廚房備餐單（${esc(lastKitchen.date)}）</h2>
      <div class="meta">合計 ${lastKitchen.grandTotal} 份　供餐自入住日午餐起、至出住日早餐止　列印時間：${esc(new Date().toLocaleString('zh-Hant-TW'))}</div>
      ${lastKitchen.vendors.map(sec).join('')}
      <div class="noprint" style="margin-top:18px;text-align:center">
        <button onclick="window.print()" style="padding:10px 24px;font-size:15px">列印 / 另存 PDF（建議橫向）</button>
      </div></body></html>`);
    win.document.close();
  };
  // 匯出 Excel（CSV）：各廠商核餐單合併一張表，前置「核餐單」欄
  $('#ml-xlsx').onclick = () => {
    if (!lastKitchen || !lastKitchen.vendors.length) return alert('當日尚無訂餐');
    const rows = [];
    for (const v of lastKitchen.vendors) {
      for (const r of v.rows) rows.push([v.choice, ...r]);
      rows.push([v.choice, '', '', '餐點小數', v.tot.breakfast, v.tot.lunch, v.tot.dinner, '', '', '', `共 ${v.total} 份`]);
    }
    downloadCsv(`廚房備餐單_${lastKitchen.date}.csv`, ['核餐單', ...KITCHEN_COLS], rows);
  };
...
// 紙本合約列印：由客戶管理合約資料組版，另開視窗列印／另存 PDF
function printPaperContract(d) {
  ...
  const items = (ct.items || []).map((it, i) => `
    <tr><td style="text-align:center">${i + 1}</td><td>${esc(it.name)}</td>
      ...
      <td style="text-align:right">$${((Number(it.qty) || 0) * (Number(it.price) || 0)).toLocaleString()}</td></tr>`).join('');
  const kv = (label, val) => `<div class="kv"><span>${esc(label)}</span><b>${esc(val || '—')}</b></div>`;
  const win = window.open('', '_blank');
  win.document.write(`<!DOCTYPE html><html lang="zh-Hant"><head><meta charset="UTF-8">
    <title>入住服務合約書 ${esc(ct.contract_no)}</title>
    <style>
      body{font-family:"Microsoft JhengHei","PingFang TC",sans-serif;color:#1c2b29;margin:32px;font-size:13.5px;line-height:1.7}
      h1{font-size:20px;text-align:center;margin:0 0 2px} .sub{text-align:center;color:#555;margin-bottom:16px;font-size:12px}
      h3{font-size:14.5px;border-left:4px solid #2c6e63;padding-left:8px;margin:18px 0 8px}
      .grid{display:grid;grid-template-columns:1fr 1fr;gap:2px 30px}
      .kv{display:flex;gap:8px;padding:2px 0} .kv span{color:#555;min-width:110px}
      table{border-collapse:collapse;width:100%;font-size:13px;margin:6px 0}
      th,td{border:1px solid #9db5b0;padding:6px 8px} th{background:#eef5f4}
      .total{text-align:right;font-size:15px;font-weight:700;margin:6px 0 0}
      .terms{color:#444;font-size:12px}
      .signs{display:grid;grid-template-columns:1fr 1fr;gap:40px;margin-top:44px}
      .signs .box{border-top:1px solid #333;padding-top:6px}
      @media print{.noprint{display:none}}
    </style></head><body>
    <h1>${esc(center)}　入住服務合約書</h1>
    <div class="sub">合約編號：${esc(ct.contract_no)}${ct.status === 'cancelled' ? '（已退訂）' : ''}　列印時間：${esc(new Date().toLocaleString('zh-Hant-TW'))}</div>
    <h3>基本資料</h3>
    <div class="grid">
      ${kv('媽媽姓名', m.name)}${kv('聯絡電話', m.phone)}
      ${kv('簽約日期', cd.sign_date)}${kv('經手人', cd.handler)}
      ${kv('預產期', cd.due_date || m.due_date)}${kv('生產胎次', cd.parity_no)}
    ...
    <table><thead><tr><th style="width:44px">項次</th><th>銷售品名</th><th style="width:80px">天數</th><th style="width:110px">單價（/日）</th><th style="width:110px">小計</th></tr></thead>
      <tbody>${items || '<tr><td colspan="5" style="text-align:center;color:#888">無明細</td></tr>'}</tbody></table>
    <div class="total">合約總額：$${(ct.total || 0).toLocaleString()}</div>
    <h3>優惠與贈品</h3>
    <div class="grid">
      ${kv('現金折扣', cd.cash_discount ? `$${Number(cd.cash_discount).toLocaleString()}` : '')}
      ${kv('商品禮券', cd.voucher_amount ? `$${Number(cd.voucher_amount).toLocaleString()}（限折抵商城商品，出住日後歸零）` : '')}
      ${kv('贈品內容', cd.gift_content)}
    </div>
    <h3>其他約定</h3>
    <div class="terms">
      <div>一、訂金為合約總額 10%（不扣現金折扣），簽約時繳付；餘款依館方規定期程繳納。</div>
      <div>二、媽媽飲食禁忌：${esc(cd.diet_ban || m.diet_notes || '無')}</div>
      <div>三、合約備註：${esc(cd.note || '無')}</div>
    </div>
    <div class="signs">
      <div class="box">立約人（甲方）簽章：<br><br>日期：　　　年　　月　　日</div>
      <div class="box">${esc(center)}（乙方）簽章：<br><br>日期：　　　年　　月　　日</div>
    </div>
...
async function viewPrograms() {
  const isAdmin = currentUser.role === 'admin';
  // 外部廠商帳號（service_scope）：只看得到自己服務的清單與報名，可新增登記、不可確認/取消（涉及入帳）
  const scope = currentUser.service_scope || '';
  ...
  const suRows = signups.length ? signups.map(s => `
    <tr>
      ...
      <td data-label="操作">${scope ? '<small style="color:var(--muted)">由館方處理</small>' : `<button class="btn small" data-confirm="${s.id}">確認</button> <button class="btn small danger" data-cancel="${s.id}">取消</button>`}</td>
  ...
  main().innerHTML = `
    ...
    <div class="card">
      <div class="row" style="justify-content:space-between;align-items:center">
        <h3 style="color:var(--primary-dark);font-size:1rem;margin:0">${scope ? `加購服務：${esc(scope)}` : '課程／服務項目'}</h3>
        <div class="row">
          ${scope ? '' : '<button class="btn small secondary" id="pg-neworder">代客報名</button>'}
          ${isAdmin || scope ? '<button class="btn small" id="pg-new">新增項目</button>' : ''}
        </div>
      </div>
  ...
  const orderBtn = main().querySelector('#pg-neworder');
  if (orderBtn) orderBtn.onclick = () => openSignupForm(progs.filter(p => p.active));
  main().querySelectorAll('[data-photos]').forEach(b => b.onclick = () => openProgramPhotos(progs.find(p => p.id == b.dataset.photos)));
  const newBtn = main().querySelector('#pg-new');
  if (newBtn) newBtn.onclick = () => openProgramForm(null, distinctCats(progs));
  if (isAdmin) {
...
async function openProgramForm(p, cats, onSaved) {
  ...
  // 外部廠商帳號：類型固定加購服務、名稱固定為其服務範圍
  const scope = currentUser.service_scope || '';
  if (scope) { ed.kind = 'service'; if (!ed.id) ed.name = scope; }
  ...
  // 既有時間拆為日期（月曆點選）＋時間（自行輸入）
  const edDate = /^\d{4}-\d{2}-\d{2}/.test(ed.scheduled_at || '') ? ed.scheduled_at.slice(0, 10) : '';
  const edTime = (ed.scheduled_at || '').slice(11, 16);
  openModal(ed.id ? '編輯課程／服務' : '新增課程／服務', `
    <div class="form-grid">
      <div class="field"><label>類型</label><select id="pg-kind" ${scope ? 'disabled' : ''}>
      ...
      <div class="field" id="pg-name-svc-wrap" style="display:none"><label>名稱 *</label>
        <select id="pg-name-svc" ${scope ? 'disabled' : ''}>
          ${scope && !SERVICE_NAME_OPTIONS.includes(scope) ? `<option selected>${esc(scope)}</option>` : ''}
          ${SERVICE_NAME_OPTIONS.map(n => `<option ${ed.name === n ? 'selected' : ''}>${n}</option>`).join('')}
          ${scope ? '' : `<option value="__other" ${ed.id && ed.kind === 'service' && !edSvcPreset ? 'selected' : ''}>其他（自行輸入）</option>`}
        </select></div>
      ...
      <div class="field"><label>日期（課程填，服務可空）</label><input type="date" id="pg-when-date" value="${esc(edDate)}"></div>
      <div class="field"><label>時間<small>（自行輸入，例：14:00）</small></label><input id="pg-when-time" value="${esc(edTime)}" placeholder="14:00" maxlength="5" inputmode="numeric"></div>
    ...
    v('#pg-save').onclick = async () => {
      ...
      // 日期用月曆點選、時間自行輸入；有填時間須同時有日期且為 HH:MM
      const whenDate = v('#pg-when-date').value;
      const whenTime = v('#pg-when-time').value.trim();
      if (whenTime && !whenDate) { v('#pg-err').textContent = '有填時間時請一併選擇日期'; return; }
      if (whenTime && !/^([01]\d|2[0-3]):[0-5]\d$/.test(whenTime)) { v('#pg-err').textContent = '時間格式請用 24 小時制 HH:MM，例：14:00'; return; }
      const payload = { kind: v('#pg-kind').value, name,
        price: Number(v('#pg-price').value) || 0, capacity: Number(v('#pg-cap').value) || 0,
        scheduled_at: whenDate ? (whenDate + (whenTime ? ` ${whenTime}` : '')) : '',
        location: v('#pg-loc').value.trim(),
        ...
        <tbody>${users.map(u => `
          <tr data-filter="${esc(u.username + ' ' + u.name)}" data-status="${u.role}"${u.active ? '' : ' style="opacity:.55"'}>
            ...
            <td data-label="可用模組" style="font-size:.82rem">${u.role === 'admin' ? '全部'
              : (u.permissions.length ? u.permissions.map(k => esc((modules.find(m => m.key === k) || {}).label || k)).join('、') : '<span style="color:var(--danger)">未授權</span>')}
              ${u.service_scope ? `<br><span class="badge yellow">廠商：${esc(u.service_scope)}</span>` : ''}</td>
...
function openUserForm(u) {
  ...
  openModal(isEdit ? `編輯帳號：${ed.username}` : '新增帳號', `
    <div class="form-grid">
      ...
      <div class="field" id="u-scope-wrap"><label>外部廠商服務範圍<small>（協力廠商帳號填其加購服務名稱；留空＝一般員工）</small></label>
        <input id="u-scope" list="u-scope-opts" maxlength="30" value="${esc(ed.service_scope || '')}" placeholder="例：泌乳">
        <datalist id="u-scope-opts">${SERVICE_NAME_OPTIONS.map(n => `<option value="${n}">`).join('')}</datalist></div>
    ...
    const syncRole = () => {
      ...
      permWrap.style.display = isAdm ? 'none' : '';
      body.querySelector('#u-scope-wrap').style.display = isAdm ? 'none' : '';
    };
    ...
    body.querySelector('#u-save').onclick = async () => {
      ...
      const payload = { name: body.querySelector('#u-name').value.trim(), phone: body.querySelector('#u-phone').value.trim(),
        ...
        service_scope: body.querySelector('#u-scope').value.trim() };
...
async function viewRoomTimeline() {
  ...
  const data = await api(`/room-calendar?start=${start}&days=${days}&include_daycare=1`);
  const dayList = [];
...
// 房況卡片「寶寶尚未登記」時的快速登記寶寶；完整報喜（餐別／哺乳衣／車號／自動排餐與備房）請至客戶管理→寶寶報喜
function openBabyRegister(motherId, motherName, onDone) {
  openModal(`登記寶寶 — ${esc(motherName)}`, `
    <div class="form-grid">
      <div class="field"><label>姓名／暱稱<small>（可留空，預設帶入媽媽姓名）</small></label><input id="qb-name" placeholder="${esc(motherName)}寶"></div>
      <div class="field"><label>性別</label>
        <select id="qb-gender"><option value="">未填</option><option value="male">男</option><option value="female">女</option></select></div>
      <div class="field"><label>出生日期</label><input type="date" id="qb-birth"></div>
      <div class="field"><label>出生體重 (g)</label><input type="number" id="qb-weight" inputmode="numeric"></div>
      <div class="field full"><label>備註</label><input id="qb-notes"></div>
      <div class="field full" style="font-size:.85rem;color:var(--muted)">完整報喜（含餐別／哺乳衣／車號，並自動帶入訂餐與請備房）請至「客戶管理 → 寶寶報喜」。此處僅快速建立寶寶資料。</div>
      <div class="full row"><button class="btn" id="qb-save">儲存</button><span class="error-msg" id="qb-err"></span></div>
    </div>`, body => {
    body.querySelector('#qb-save').onclick = async () => {
      try {
        await api('/babies', { method: 'POST', body: {
          mother_id: motherId,
          name: body.querySelector('#qb-name').value.trim() || `${motherName}寶`,
          gender: body.querySelector('#qb-gender').value,
          birth_date: body.querySelector('#qb-birth').value,
          birth_weight_g: Number(body.querySelector('#qb-weight').value) || null,
          notes: body.querySelector('#qb-notes').value
        } });
        closeModal();
        if (onDone) onDone();
      } catch (e) { body.querySelector('#qb-err').textContent = e.message; }
    };
  });
}
...
async function viewMotherRooms() {
  ...
  main().querySelectorAll('[data-add-baby]').forEach(btn =>
    btn.onclick = () => openBabyRegister(btn.dataset.addBaby, btn.dataset.mom, viewMotherRooms));
  // 快速新增房務任務
...
async function viewMotherUpcoming(kind) {
  ...
  const cols = isIn
    ? ['房號', '媽媽姓名', '生日', '後四碼', '入住日', '出住日', '總天數', '生產日期', '生產方式', '生產醫院', '胎次', '妊娠週數', '寶寶性別', '寶寶體重', '狀態']
    : ['預退日', '房號', '房型', '媽媽', '電話', '入住日', '已住天數', '狀態'];
  const tr = r => {
    const total = dayDiff(r.check_in, r.check_out);
    // 性別／體重分開兩欄；多胞胎每胎各一列（以 <br> 對齊）
    const babyGender = (r.babies || []).map(b =>
      b.gender === 'male' ? '<span style="color:#3b78c2">♂男</span>' : b.gender === 'female' ? '<span style="color:var(--accent)">♀女</span>' : '—').join('<br>') || '—';
    const babyWeight = (r.babies || []).map(b => b.birth_weight_g ? `${b.birth_weight_g} g` : '—').join('<br>') || '—';
    const cells = isIn
      ? [esc(r.room_name), `${esc(r.mother_name)}${r.phone ? `<br><small>${esc(r.phone)}</small>` : ''}`,
         esc(r.mother_birth || '—'), esc(r.id4 || '—'), esc(r.check_in), esc(r.check_out), `${total} 天`,
         esc(r.delivery_date || '—'), esc(r.delivery_type || '—'), esc(r.birth_hospital || '—'),
         esc(r.parity_no || '—'), esc(r.weeks || '—'), babyGender, babyWeight, badge(r)]
...
async function viewCareRecordQuery(kind) {
  ...
  const st = CRQ_STATE[kind] || (CRQ_STATE[kind] = { start: todayStr().slice(0, 8) + '01', end: todayStr(), kw: '', kwtype: 'name', status: 'all' });
  main().innerHTML = `
    <div class="page-title">${title} <small style="font-weight:400;color:var(--muted);font-size:.85rem">（可查詢入住中與已出住的照護資料）</small></div>
    <div class="card">
      <div class="sec-hd">${title}（資料查詢）</div>
      <div class="form-grid" style="align-items:end">
        ...
        <div class="field"><label>住客狀態</label>
          <div class="row" style="gap:14px;padding-top:6px">
            <label><input type="radio" name="crq-status" value="all" ${st.status !== 'checked_in' && st.status !== 'checked_out' ? 'checked' : ''}> 全部</label>
            <label><input type="radio" name="crq-status" value="checked_in" ${st.status === 'checked_in' ? 'checked' : ''}> 入住中</label>
            <label><input type="radio" name="crq-status" value="checked_out" ${st.status === 'checked_out' ? 'checked' : ''}> 已出住</label>
          </div></div>
  ...
  const run = async () => {
    ...
    st.status = (main().querySelector('input[name="crq-status"]:checked') || {}).value || 'all';
      ...
      <div class="bbc ${hasAlert ? 'alert' : ''}" data-state="${states.join(' ')}">
        ...
        ${canCare ? `<div class="bbc-foot">
          <div class="row" style="gap:6px;flex-wrap:wrap">
            ...
            <a class="btn small" href="#/baby-guidance?b=${b.id}">寶寶護理指導</a>
            <a class="btn small" href="#/breastfeeding?b=${b.id}">母乳哺育評估</a>
            ${canAccess('#/mother-guidance') ? `<a class="btn small secondary" href="#/mother-guidance?m=${b.mother_id}">產婦護理指導</a>` : ''}
  ...
  main().innerHTML = `
    ...
    <div class="card">
      ...
      <div class="row" style="gap:6px 16px;flex-wrap:wrap;margin-top:8px;font-size:.88rem;color:var(--muted)">
        ${BABY_LEGEND.map(([label, color]) => `<span><i class="legend-sq" style="background:${color};${color === '#ffffff' ? 'border:1px solid var(--border)' : ''}"></i>${label}</span>`).join('')}
        <span style="display:inline-flex;align-items:center;gap:4px"><span style="display:inline-flex;color:var(--primary)">${BATH_SVG}</span>今日已沐浴</span>
      </div>
  ...
  main().innerHTML = `
    ...
    <div class="card no-print">
      <div class="row" style="gap:10px;align-items:flex-end;flex-wrap:wrap">
        ...
        <a class="btn small secondary" href="#/baby-guidance?b=${babyId}">寶寶護理指導</a>
...
/* ---------- 護理指導單（產婦／寶寶共用；評量項目條列＋逐項產婦手寫簽名） ---------- */
// cfg：{ base 指導單 API 前綴, itemsPath 項目主檔 API, title 單名, subjectName 對象姓名, roomName 房號, reload 重新整理 }
function guidanceSheetHtml(sheet, cfg, isAdmin) {
  ...
  const itemRows = sheet.items.map(it => {
    ...
    return `
      <tr>
        ...
        <td data-label="評量日期／簽名">${e.re_date
  ...
  return `
    <div class="card">
      <div class="sec-hd">${esc(cfg.title)}（已完成 ${doneCount}／${sheet.items.length} 項）</div>
      <div class="row" style="gap:6px 18px;flex-wrap:wrap;font-size:.95rem;margin-bottom:8px">
        <span><b>房號：</b>${esc(cfg.roomName || '—')}</span>
        <span><b>${esc(cfg.subjectLabel)}：</b>${esc(cfg.subjectName)}</span>
        ${sheet.updated_at ? `<span><b>最後更新：</b>${esc(sheet.updated_at)}</span>` : ''}
      </div>
      <div class="table-wrap">
        <table class="data stack">
          <thead><tr><th>評量項目</th><th>護理衛教指導內容</th><th>指導日期／簽名</th><th>產婦簽名</th><th>評量結果</th><th>評量日期／簽名</th><th class="no-print">狀態</th><th class="no-print"></th></tr></thead>
      ...
      <div style="margin-top:8px;font-size:.82rem;color:var(--muted)">
        評量結果：${Object.values(sheet.result_labels || {}).map(esc).join('　')}</div>
    </div>`;
}

// 綁定指導簽名／再評量簽名／清除；簽名框為產婦手寫，日期與指導者於儲存時自動帶入
function wireGuidanceSheet(sheet, cfg) {
  const entries = sheet.entries || {};
  main().querySelectorAll('[data-sign]').forEach(btn => btn.onclick = () => {
    ...
    const curResult = stage === 're' ? e.re_result : e.result;
    openModal(`${stage === 're' ? '再評量簽名' : '指導簽名'} — ${esc(cfg.subjectName)}`, `
      <p style="margin:0 0 8px"><b>${esc(item.category)}</b>　${esc(item.name)}</p>
      <div class="form-grid">
        <div class="field"><label>${stage === 're' ? '評量' : '指導'}日期</label>
          <input type="date" id="gs-date" value="${todayStr()}"></div>
        <div class="field"><label>評量結果</label>
          <select id="gs-result"><option value="">請選擇</option>${sheet.results.map(o =>
      `<option value="${esc(o)}" ${o === curResult ? 'selected' : ''}>${esc((sheet.result_labels || {})[o] || o)}</option>`).join('')}</select></div>
      ...
      const pad = mountSigPad(body.querySelector('#gs-pad'));
      ...
      body.querySelector('#gs-save').onclick = async () => {
        ...
        try {
          await api(`${cfg.base}/${item.id}`, { method: 'PUT', body: {
  ...
  main().querySelectorAll('[data-clear]').forEach(btn => btn.onclick = async () => {
    ...
    await api(`${cfg.base}/${btn.dataset.clear}`, { method: 'DELETE' });
    cfg.reload();
  });
}

// 評量項目主檔維護（管理員）：可增列、改內容、刪除、調整順序
async function openGuidanceItems(cfg) {
  const list = await api(cfg.itemsPath);
  const row = it => `
    <tr>
  ...
  openModal(`維護評量項目 － ${esc(cfg.title)}`, `
    <p style="font-size:.85rem;color:var(--muted);margin:0 0 8px">
    ...
    body.querySelector('#gi-save').onclick = async () => {
      ...
      try {
        await api(cfg.itemsPath, { method: 'PUT', body: { items: rows } });
        closeModal(); cfg.reload();
...
function printGuidanceSheet(sheet, cfg) {
  ...
  const win = window.open('', '_blank');
  win.document.write(`<!DOCTYPE html><html lang="zh-Hant"><head><meta charset="UTF-8">
    <title>${esc(cfg.title)} - ${esc(cfg.subjectName)}</title>
    <style>
      body{font-family:"Microsoft JhengHei","PingFang TC",sans-serif;color:#1c2b29;line-height:1.5;max-width:900px;margin:24px auto;padding:0 24px}
      h1{font-size:18px;margin:0 0 2px;text-align:center}
      ...
      .note{margin-top:10px;font-size:11.5px;color:#333;line-height:1.6}
      @media print{.noprint{display:none}}
    </style></head><body>
    <h1>${esc(center)}</h1>
    <div class="sub">${esc(cfg.title)}</div>
    <div class="hd">房號：${esc(cfg.roomName || '')}　${esc(cfg.subjectLabel)}：${esc(cfg.subjectName)}</div>
    <table>
      <thead><tr><th style="width:70px">評量項目</th><th>護理衛教指導內容</th><th style="width:90px">指導日期／簽名</th><th style="width:90px">產婦簽名</th><th style="width:70px">評量結果</th><th style="width:90px">評量日期／簽名</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
    <div class="note">備註：指導／評值對象「新入住或轉入者之產婦」，指導／評值時間「入住～出住」，採不定期指導／評值。<br>
      填表說明：學習成效請以號呈現　${Object.values(sheet.result_labels || {}).map(esc).join('；')}</div>
    <div class="noprint" style="margin-top:20px;text-align:center"><button onclick="window.print()" style="padding:10px 24px;font-size:15px">列印 / 另存 PDF</button></div>
    </body></html>`);
  win.document.close();
}

/* ---------- 護理指導（產婦；獨立頁，資料與媽媽護理頁共用） ---------- */
async function viewMotherGuidance() {
  ...
  const momId = mothers.some(m => m.id === want) ? want : mothers[0].id;
  const [{ mother, reminders }, sheet] = await Promise.all([
    api(`/mothers/${momId}/guidance`), api(`/mothers/${momId}/guidance-sheet`)
  ]);
  const kindLabel = k => k === 'care' ? '產婦護理衛教指導單' : '母乳哺育評估單';
  const isAdmin = currentUser.role === 'admin';
  const cfg = {
    base: `/mothers/${momId}/guidance-sheet`, itemsPath: '/guidance-items',
    title: '產婦護理衛教指導單', subjectLabel: '產婦', subjectName: mother.name,
    roomName: mother.room_name, reload: viewMotherGuidance
  };
  ...
  main().innerHTML = `
    ...
    ${guidanceSheetHtml(sheet, cfg, isAdmin)}
    <div class="card">
  ...
  $('#mgl-print').onclick = () => printGuidanceSheet(sheet, cfg);
  if (isAdmin) $('#mgl-items').onclick = () => openGuidanceItems(cfg);
  wireGuidanceSheet(sheet, cfg);
...
/* ---------- 寶寶護理指導（新生兒護理衛教指導單；比照產婦頁） ---------- */
async function viewBabyGuidance() {
  const list = await api('/room-status/babies');
  const babies = list.babies;
  if (!babies.length) {
    main().innerHTML = '<div class="page-title">寶寶護理指導</div><div class="card"><div class="empty">目前沒有在住寶寶</div></div>';
    return;
  }
  const want = Number((location.hash.split('?b=')[1] || '').split('&')[0]);
  const babyId = babies.some(b => b.id === want) ? want : babies[0].id;
  const sheet = await api(`/babies/${babyId}/guidance-sheet`);
  const baby = sheet.baby;
  const isAdmin = currentUser.role === 'admin';
  const cfg = {
    base: `/babies/${babyId}/guidance-sheet`, itemsPath: '/baby-guidance-items',
    title: '新生兒護理衛教指導單', subjectLabel: '寶寶',
    subjectName: `${baby.name}（媽媽：${baby.mother_name}）`,
    roomName: baby.room_name, reload: viewBabyGuidance
  };

  main().innerHTML = `
    <div class="page-title">寶寶護理指導 <small style="font-weight:400;color:var(--muted);font-size:.9rem">新生兒護理衛教指導單</small></div>
    <div class="card no-print">
      <div class="row" style="gap:10px;align-items:flex-end;flex-wrap:wrap">
        <div class="field" style="max-width:260px;margin:0"><label>選擇寶寶</label>
          <select id="bgl-baby">${babies.map(b => `<option value="${b.id}" ${b.id === babyId ? 'selected' : ''}>${esc(b.name)}（${esc(b.mother_name)}${b.room_name ? `／${esc(b.room_name)}` : ''}）</option>`).join('')}</select></div>
        <a class="btn small secondary" href="#/baby-rooms">回寶寶房況</a>
        <a class="btn small secondary" href="#/baby-nursing?b=${babyId}">寶寶護理</a>
        <a class="btn small secondary" href="#/breastfeeding?b=${babyId}">母乳哺育評估</a>
        <button class="btn small secondary" id="bgl-print">列印指導單</button>
        ${isAdmin ? '<button class="btn small secondary" id="bgl-items">維護評量項目</button>' : ''}
      </div>
    </div>
    ${guidanceSheetHtml(sheet, cfg, isAdmin)}`;

  $('#bgl-baby').onchange = () => { location.hash = `#/baby-guidance?b=${$('#bgl-baby').value}`; };
  $('#bgl-print').onclick = () => printGuidanceSheet(sheet, cfg);
  if (isAdmin) $('#bgl-items').onclick = () => openGuidanceItems(cfg);
  wireGuidanceSheet(sheet, cfg);
}
/* ---------- 產婦結案 ---------- */
...
async function viewCustomers() {
  ...
  function wireContract(d) {
    ...
    const ctPayload = () => {
      ...
      for (const [sel, key, byKey] of [['#ct-voucher', 'voucher_amount', 'voucher_by'],
        ['#ct-cashdisc', 'cash_discount', 'cash_discount_by'], ['#ct-gift', 'gift_content', 'gift_by']]) {
        const v = gv(sel);
        if (v !== String(cd[key] ?? '')) { p[key] = v; p[byKey] = currentUser.name; }
      }
      return p;
    };
    // 必填檢核（第一次建立與後續修改皆須齊全）
    const ctValidate = () => {
      ...
      if (!d.contract || !d.contract.items.length) missing.push('銷售房型明細（至少一筆）');
      return missing;
    };
    const doSave = async errSel => {
      const err = $q(errSel);
      err.textContent = '';
      const missing = ctValidate();
      if (missing.length) { err.textContent = `必填未完成：${missing.join('、')}`; return; }
      try { await cput(ctPayload()); } catch (e) { err.textContent = e.message; }
    };
    $q('#ct-save').onclick = () => doSave('#ct-err');
    // 頁尾「儲存合約」：同資料存檔／修改（表單很長，底部再放一顆方便存檔）
    const saveAll = $q('#ct-save-all');
    if (saveAll) saveAll.onclick = () => doSave('#ct-err2');
    const printPaper = $q('#ct-print-paper');
    if (printPaper && !printPaper.disabled) printPaper.onclick = () => printPaperContract(d);
  ...
  async function openBabyAnnounce(d, birth) {
    ...
    const ctDays = ctItems.reduce((s, it) => s + (Number(it.qty) || 0), 0);
    const stayDays = ctDays || (bk.check_in && bk.check_out ? Math.round((new Date(bk.check_out) - new Date(bk.check_in)) / 86400000) : '');
    let roomList = [];
    if (!bk.id) { try { roomList = (await api('/rooms')).filter(r => r.active); } catch (e) { roomList = []; } }
    const defIn = cd.expected_check_in || '';
    const defOut = defIn && ctDays ? new Date(new Date(defIn + 'T00:00:00Z').getTime() + ctDays * 86400000).toISOString().slice(0, 10) : (cd.expected_check_out || '');
    const TABOOS = ['無', '牛肉', '羊肉', '內臟', '帶殼海鮮'];
    // 禁忌食材：從合約「媽媽飲食禁忌」自動帶入（比對勾選，其餘放「其他」），可手改
    const banMatched = new Set();
    const banOthers = [];
    for (const tok of String(cd.diet_ban || '').split(/[、,，\/\s]+/).map(s => s.trim()).filter(Boolean)) {
      const hit = TABOOS.find(t => t !== '無' && (tok.includes(t) || t.includes(tok)));
      if (hit) banMatched.add(hit); else banOthers.push(tok);
    }
    const babyRow = i => `
    ...
    openModal(`寶寶報喜：${m.name}（入住通知單）`, `
      <div class="form-grid">
        ...
        <div class="field"><label>禁忌食材<small>（合約飲食禁忌自動帶入，可手改）</small></label>
          <div class="row" style="gap:10px;padding-top:6px;flex-wrap:wrap">${TABOOS.map(t =>
            `<label class="bna-chk"><input type="checkbox" data-ba-taboo value="${t}" ${banMatched.has(t) ? 'checked' : ''}> ${t}</label>`).join('')}
            <input id="ba-taboo-other" placeholder="其他" value="${esc(banOthers.join('、'))}" style="max-width:140px"></div></div>
      ...
      // 手 key 模式：入住日改動時，出住日自動依合約總天數帶入（可再手改）
      const inEl = body.querySelector('#ba-in');
      if (inEl) inEl.onchange = () => {
        if (!inEl.value || !ctDays) return;
        body.querySelector('#ba-out').value = new Date(new Date(inEl.value + 'T00:00:00Z').getTime() + ctDays * 86400000).toISOString().slice(0, 10);
      };
      body.querySelector('#ba-save').onclick = async () => {
        ...
        if (bk.id) {
          ...
          const sel = body.querySelector('#ba-room-sel');
          const roomId = Number(sel.value) || 0;
          bkIn = inVal;
          bkOut = outVal;
          if (roomId && bkIn && bkOut) {
            try {
              await api('/bookings', { method: 'POST', body: {
                mother_id: editId, room_id: roomId, check_in: bkIn, check_out: bkOut,
                deposit: 0, total_amount: (d.contract && d.contract.total) || 0, notes: '寶寶報喜排房帶入'
              } });
              bkRoom = sel.selectedOptions[0].dataset.name || '';
            } catch (e) { err.textContent = `排房未完成：${e.message}`; return; }
          } else if (roomId || bkIn) {
            err.textContent = '手動排房請同時選擇房號並填寫入住日／出住日'; return;
          }
        }
        const payload = {
        ...
        try {
          ...
          closeModal();
          openModal('寶寶報喜完成', `
            <p>已儲存入住通知單${r.created_babies ? `，並建立 ${r.created_babies} 筆寶寶資料` : ''}。</p>
            ${r.hk_task ? `<p style="font-size:.85rem;margin:4px 0">已自動建立房務任務「${esc(r.hk_task)}」（排定於今日）${r.meal_date ? `，並自 ${esc(r.meal_date)} 入住日午餐起帶入訂餐至 ${esc(r.meal_end || '')} 出住日早餐（餐別／禁忌）` : ''}。</p>` : ''}
            <p style="margin:4px 0 0">資料已轉入下列模組：</p>
            ...
            mBody.querySelector('#ba-print').onclick = () => printBabyAnnounce(m,
              { ...bk, room_name: bkRoom || bk.room_name, check_in: bkIn || bk.check_in, check_out: bkOut || bk.check_out },
              birth, cd, payload, stayDays);
          });
  ...
  async function wirePanels(d) {
    ...
    if (bkRows) {
      ...
      // 房型定價（判斷升等／降等）；優先取房型設定，退回房間每日房價
      const allTypes = [...new Set([...(d.room_types || []).map(t => t.name), ...roomList.map(r => r.room_type)])].filter(Boolean);
      const typePrice = name => {
        const t = (d.room_types || []).find(x => x.name === name);
        if (t) return Number(t.price) || 0;
        const rm = roomList.find(r => r.room_type === name);
        return rm ? (Number(rm.price_per_day) || 0) : 0;
      };
      // 安排房型與預定房型不同時，於該列顯示升等／降等警語
      const updateWarn = tr => {
        const warnEl = tr.querySelector('[data-bk-warn]');
        if (!warnEl) return;
        const planned = tr.dataset.plannedType || '';
        const assigned = tr.dataset.type || '';
        if (planned && assigned && assigned !== planned) {
          const diff = typePrice(assigned) - typePrice(planned);
          const grade = diff > 0 ? '升等' : diff < 0 ? '降等' : '房型異動';
          tr.dataset.grade = grade;
          warnEl.textContent = `⚠ 與預定「${planned}」不同（${grade}）`;
          warnEl.style.color = diff > 0 ? '#c0392b' : diff < 0 ? '#b9911f' : 'var(--muted)';
        } else {
          tr.dataset.grade = '';
          warnEl.textContent = '';
        }
      ...
      const wireRow = tr => {
        ...
        if (typeSel) typeSel.onchange = () => {
          tr.dataset.type = typeSel.value;
          roomSel.innerHTML = roomOpts(typeSel.value);
          updateWarn(tr);
        };
        const del = tr.querySelector('[data-bk-del]');
      ...
      $q('#bk-addrow').onclick = () => {
        ...
        tr.dataset.type = allTypes[0] || '';
        tr.dataset.plannedType = ''; // 加開列無預定房型，不做升／降判斷
        tr.innerHTML = `
          <td data-label="預定房型"><small style="color:var(--muted)">（加開）</small></td>
          <td data-label="預定天數">—</td>
          <td data-label="安排房型"><select data-bk-type style="min-width:120px">${allTypes.map(t => `<option>${esc(t)}</option>`).join('')}</select></td>
    ...
    if (bcCard) {
      ...
      const drawChart = async () => {
        ...
        let cal;
        try { cal = await api(`/room-calendar?start=${startD}&days=60&include_daycare=1`); }
        catch (e) { $q('#bc2-chart').innerHTML = `<div class="empty">${esc(e.message)}</div>`; return; }
        const days = [];
        for (let i = 0; i < cal.days; i++) days.push(new Date(new Date(cal.start).getTime() + i * 86400000).toISOString().slice(0, 10));
        const td = todayStr();
        const ctItems = (d.contract && d.contract.items) || [];
        // 升等／降等以「房型設定」定價判斷：合約銷售房型定價 vs 實際排房房型定價
        const typePrice = name => {
          const t = (d.room_types || []).find(x => x.name === name);
          return t ? (Number(t.price) || 0) : 0;
        };
        const basePrice = ctItems.length ? Math.max(...ctItems.map(i => typePrice(i.name) || Number(i.price) || 0)) : 0;
        // 本客戶的訂房標示：房型符合＝不標、升＝升等、降＝降等、特＝特殊需求（訂房有備註內容）
        const markFor = bk => {
          ...
          if (room && basePrice && !ctItems.some(i => i.name === room.room_type)) {
            marks.push((typePrice(room.room_type) || room.price_per_day || 0) > basePrice ? '升' : '降');
          }
          if (String(bk.notes || '').trim()) marks.push('特');
        ...
        const rowsHtml = cal.rooms.map(r => {
          const cells = days.map(dd => {
            const covers = cal.bookings.filter(b => b.room_id === r.id && b.check_in <= dd && b.check_out > dd);
            const bkIn2 = covers.find(b => b.status === 'checked_in');
            const bkRes2 = covers.find(b => b.status === 'reserved');
            const bk = bkIn2 || covers[0];
            if (bk) {
              const isStart = bk.check_in === dd || dd === cal.start;
              if (bcTab === 'actual' && bkIn2 && bkRes2) {
                return `<td title="重疊！入住中 ${esc(bkIn2.mother_name)}（~${esc(bkIn2.check_out)}）與已預約 ${esc(bkRes2.mother_name)}（${esc(bkRes2.check_in)}~）同日同房" style="background:repeating-linear-gradient(45deg,#f6c6c6,#f6c6c6 4px,#cdeae4 4px,#cdeae4 8px);padding:2px;font-size:10px;color:#b23b3b;font-weight:700;white-space:nowrap;overflow:hidden;max-width:0">${isStart || bkRes2.check_in === dd ? '疊' : ''}</td>`;
              }
              if (bcTab === 'actual' && !bkIn2 && bk.status === 'reserved') {
              ...
              return `<td title="${esc(bk.mother_name)}（${esc(bk.check_in)}~${esc(bk.check_out)}・${STATUS_LABEL[bk.status]}）${mk}${bk.notes ? `　備註：${esc(bk.notes)}` : ''}" style="background:${color};padding:2px;font-size:11px;white-space:nowrap;overflow:hidden;max-width:0;${mine ? 'outline:2px solid var(--primary-dark);outline-offset:-2px;' : ''}">${isStart ? esc(mk + bk.mother_name.slice(0, 4)) : ''}</td>`;
            }
        ...
        $q('#bc2-legend').innerHTML = bcTab === 'planned'
          ? '<span class="badge green">入住中</span> <span class="badge yellow">已預約</span> <span class="badge gray">已退住</span>　〔升〕升等　〔降〕降等　〔特〕特殊需求　粗框＝本客戶'
          : '<span class="badge green">入住中</span> <span class="badge gray">已退住</span>　斜線格＝已預約尚未入住　<span style="color:#b23b3b;font-weight:700">紅斜線〔疊〕＝入住中與已預約重疊</span>';
        $q('#bc2-chart').innerHTML = `
          <div class="row between"><h3 style="margin:0;font-size:.95rem">${bcTab === 'planned' ? '預定床表' : '實際入住床表'}（${esc(cal.start)} 起 60 天）</h3></div>
          <div class="table-wrap" style="overflow-x:scroll;margin-top:6px;padding-bottom:4px"><table style="border-collapse:collapse;font-size:12px;min-width:${140 + cal.days * 34}px">
            <thead><tr><th style="position:sticky;left:0;background:#eef5f4;padding:2px 6px">房號</th>${headCells}</tr></thead>
    ...
    <div class="cpanel" data-tab="rooms">
      ...
      ${canAccess('#/rooms') ? (() => {
        ...
        <div class="table-wrap"><table class="data" id="bk-rows-table">
          <thead><tr><th>預定房型</th><th>預定天數</th><th>安排房型 <b class="req">*</b></th><th>房號 <b class="req">*</b></th><th>天數 <b class="req">*</b></th><th></th></tr></thead>
          <tbody id="bk-rows">${bct.items.map(it => `
            <tr data-bk-row data-type="${esc(it.name)}" data-planned-type="${esc(it.name)}">
              <td data-label="預定房型">${esc(it.name)}<br><small>$${(it.price || 0).toLocaleString()}/日</small></td>
              <td data-label="預定天數">${it.qty} 天</td>
              <td data-label="安排房型"><select data-bk-type style="min-width:120px">${((d.room_types && d.room_types.length) ? d.room_types.map(t => t.name) : [it.name]).map(tn => `<option ${tn === it.name ? 'selected' : ''}>${esc(tn)}</option>`).join('')}</select><br><small data-bk-warn style="font-size:.72rem"></small></td>
        ...
        <div class="field full" style="margin-top:8px"><label>備註<small>（有填＝床表標示特殊需求）</small></label>
          <input id="bk-note" maxlength="200" placeholder="例：靠電梯、需相通房"></div>
    ...
    <div class="cpanel" data-tab="contracts">
      ...
      <div class="card">
        ...
        <div class="form-grid">
          ...
          <div class="field"><label>預計入住日 <b class="req">*</b></label><input type="date" id="ct-expin" data-days="${ct ? ct.items.reduce((s, it) => s + (Number(it.qty) || 0), 0) : 0}" value="${esc(cd.expected_check_in || '')}"></div>
          ...
          <div class="field"><label>生產胎次 <b class="req">*</b></label><select id="ct-parity"><option value="">--請選擇--</option>${parityOpts.map(o => `<option ${cd.parity_no === o ? 'selected' : ''}>${o}</option>`).join('')}</select></div>
          <div class="field"><label>寶寶人數 <b class="req">*</b></label>
          ...
          <div class="field"><label>媽媽手機 <b class="req">*</b></label><input id="ct-mphone" maxlength="20" value="${esc(m.phone || '')}" placeholder="0912345678"></div>
      ...
      <div class="card no-print">
        <div class="row" style="gap:10px;flex-wrap:wrap;align-items:center">
          <button class="btn danger" id="ct-save-all">儲存合約</button>
      ...
      <div class="card">
        ...
        ${(d.contract_logs || []).length ? `<ul class="timeline" style="max-height:340px;overflow-y:auto">${d.contract_logs.map(l => {
          const body = String(l.summary || '').replace(/^客戶合約資料修改[:：]?/, '').trim();
          return `
          <li>
            <div class="time">${esc((l.created_at || '').slice(0, 16))}　${esc(l.user_name || '')}</div>
            <div class="what" style="font-weight:400;font-size:.85rem">${
              body === '（無欄位變更）' || !body ? '<span style="color:var(--muted)">存檔（無欄位變更）</span>'
              : body.split('；').map(x => esc(x)).join('<br>')}</div>
          </li>`;
...
const routes = {
  ...
  '#/equipment-repair': viewEquipmentRepair,
