  ...
  const matrix = mode => cal.rooms.map(r => {
    const cells = days.map(d => {
      ...
      if (bk) {
        ...
        return `<td title="${esc(bk.mother_name)}（${esc(bk.check_in)}~${esc(bk.check_out)}・${STATUS_LABEL[bk.status]}）" style="background:${color};padding:2px;font-size:11px;white-space:nowrap;overflow:hidden;max-width:0;${bk.status === 'checked_out' ? 'color:#8a938f;' : ''}">${isStart ? esc(bk.mother_name.slice(0, 4)) : ''}</td>`;
      }
      return mode === 'planned'
...
async function viewHousekeeping() {
  ...
  main().innerHTML = `
    ...
    <div class="card">
      ...
      <div class="table-wrap"><table class="data stack">
        ...
        <tbody>${cleanTasks.map(taskRow).join('') || '<tr><td colspan="5"><div class="empty">本日尚無清潔任務</div></td></tr>'}</tbody>
      </table></div>
    </div>
    <div class="card">
      <h3>維修任務（${esc(data.date)}）</h3>
      <p style="font-size:.8rem;color:var(--muted);margin:0 0 8px">設備報修：由「新增清潔任務」選故障類別建立；處理過程可按「進度」逐筆記錄。</p>
      <div class="table-wrap"><table class="data stack">
        <thead><tr><th>故障類別</th><th>位置</th><th>排定日</th><th>狀態</th><th></th></tr></thead>
        <tbody>${repairTasks.map(taskRow).join('') || '<tr><td colspan="5"><div class="empty">本日尚無維修任務</div></td></tr>'}</tbody>
      </table></div>
    </div>`;
  ...
  main().querySelectorAll('[data-hk-prog]').forEach(b => b.onclick = () =>
    openHkProgress(data.tasks.find(t => String(t.id) === b.dataset.hkProg)));
}
  ...
  openModal('新增清潔任務', `
    <div class="form-grid">
      <div class="field full"><label>任務</label>
        <select id="hkt-task">
          <optgroup label="清潔任務">${options.map(p => `<option value="c:${esc(p)}">${esc(p)}</option>`).join('')}
            <option value="c:__other">其他（自行說明）</option></optgroup>
          <optgroup label="設備報修（故障類別）">${HK_REPAIR_OPTIONS.map(p =>
            p === '其他' ? '<option value="r:__other">其他（自行說明）</option>' : `<option value="r:${esc(p)}">${esc(p)}</option>`).join('')}</optgroup>
        </select>
      </div>
      <div class="field full" id="hkt-other-wrap" style="display:none"><label>其他（請說明）</label><input id="hkt-other" placeholder="請描述任務或故障內容"></div>
    ...
    taskSel.onchange = () => { body.querySelector('#hkt-other-wrap').style.display = taskSel.value.endsWith('__other') ? '' : 'none'; };
    body.querySelector('#hkt-save').onclick = async () => {
      const [kindCode, ...rest] = taskSel.value.split(':');
      ...
      try {
        await api('/housekeeping/tasks', { method: 'POST', body: {
          task, kind: kindCode === 'r' ? 'repair' : 'clean',
...
async function viewMotherUpcoming(kind) {
  ...
  const cols = isIn
    ? ['房號', '媽媽姓名', '生日', '後四碼', '入住日', '出住日', '總天數', '生產日期', '生產方式', '生產醫院', '胎次', '妊娠週數', '寶寶性別／體重', '狀態']
    : ['預退日', '房號', '房型', '媽媽', '電話', '入住日', '已住天數', '狀態'];
  const tr = r => {
    const total = dayDiff(r.check_in, r.check_out);
    const babyLine = (r.babies || []).map(b =>
      `${b.gender === 'male' ? '<span style="color:#3b78c2">♂男</span>' : b.gender === 'female' ? '<span style="color:var(--accent)">♀女</span>' : '—'}${b.birth_weight_g ? ` ${b.birth_weight_g}g` : ''}`).join('<br>') || '—';
    const cells = isIn
      ? [esc(r.room_name), `${esc(r.mother_name)}${r.phone ? `<br><small>${esc(r.phone)}</small>` : ''}`,
         ...
         esc(r.parity_no || '—'), esc(r.weeks || '—'), babyLine, badge(r)]
  ...
  async function openBabyAnnounce(d, birth) {
    ...
    openModal(`寶寶報喜：${m.name}（入住通知單）`, `
      <div class="form-grid">
        ...
        <div class="field"><label>入住日<small>（可手改，儲存後寫回床表）</small></label><input type="date" id="ba-in" value="${esc(defIn)}"></div>
        <div class="field"><label>出住日<small>（依合約總天數帶入）</small></label><input type="date" id="ba-out" value="${esc(defOut)}"></div>`}
      ...
      body.querySelector('#ba-save').onclick = async () => {
        ...
        // 無排房紀錄且已手 key 房號＋入住日 → 先建立訂房（雙向寫回實際入住床表）
        ...
        if (!bk.id) {
          ...
          bkIn = body.querySelector('#ba-in').value;
          bkOut = body.querySelector('#ba-out').value;
        ...
        try {
          ...
          openModal('寶寶報喜完成', `
            ...
            ${r.hk_task ? `<p style="font-size:.85rem;margin:4px 0">已自動建立房務任務「${esc(r.hk_task)}」${r.meal_date ? `，並帶入 ${esc(r.meal_date)} 當日訂餐（餐別／禁忌）` : ''}。</p>` : ''}
