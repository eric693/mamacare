...
test('排房變更連動收費：PUT 訂房帶 total_amount 後帳務應收同步', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  const bk = (await req('GET', '/api/billing')).data.find(x => x.status === 'checked_in' || x.status === 'reserved');
  assert.ok(bk, '需有有效訂房');
  const days = Math.round((new Date(bk.check_out) - new Date(bk.check_in)) / 86400000);
  const newOut = new Date(new Date(bk.check_in).getTime() + (days + 10) * 86400000).toISOString().slice(0, 10);
  const newTotal = (days + 10) * 6800;
  const r = await req('PUT', `/api/bookings/${bk.id}`, { check_in: bk.check_in, check_out: newOut, total_amount: newTotal });
  assert.strictEqual(r.status, 200);
  ...
  // 還原，避免影響其他測試狀態
  await req('PUT', `/api/bookings/${bk.id}`, { check_in: bk.check_in, check_out: bk.check_out, total_amount: bk.total_amount });
});
