...
// 取得一位入住中媽媽；若前面的測試已把共用住客全部退房，就自行建一位（住期涵蓋今日之後）
async function anyCheckedInMother() {
  const found = (await req('GET', '/api/mothers')).data.find(m => m.status === 'checked_in');
  if (found) return found;
  const base = Date.now() - new Date().getTimezoneOffset() * 60000;
  const D = n => new Date(base + n * 86400000).toISOString().slice(0, 10);
  const c = await req('POST', '/api/customers', { name: `備援住客${Date.now() % 100000}`, due_date: D(0) });
  const rooms = (await req('GET', '/api/rooms')).data.filter(r => r.active && r.room_type !== '托嬰').reverse();
  for (const r of rooms) {
    const t = await req('POST', '/api/bookings', { mother_id: c.data.id, room_id: r.id, check_in: D(-5), check_out: D(20) });
    if (t.status === 200) {
      await req('PUT', `/api/bookings/${t.data.id}/status`, { status: 'checked_in' });
      return (await req('GET', '/api/mothers')).data.find(m => m.id === c.data.id);
    }
  }
  throw new Error('無可用房間可建立備援住客');
}
...
test('產婦結案：結案存檔→更新→房況旗標→解除結案', async () => {
  ...
  // 產婦結案不連動退房：媽媽仍為入住中、房況仍在原房號，只是掛上「已結案」旗標
  // （退房一律由住客管理「退房完成」負責，三者互不連動）
  const rs = await req('GET', '/api/room-status/mothers');
  const stillOcc = rs.data.rooms.find(r => r.occupant && r.occupant.mother_id === mom.id);
  assert.ok(stillOcc, '產婦結案後房況仍應顯示該媽媽（不連動退房）');
  assert.ok(stillOcc.occupant.closed, '房況應顯示已結案旗標');
  const mAfter = (await req('GET', '/api/mothers')).data.find(m => m.id === mom.id);
  assert.strictEqual(mAfter.status, 'checked_in', '產婦結案不得改變媽媽入住狀態');
  // 解除結案（admin）：只刪結案紀錄，不動訂房／房況
  const del = await req('DELETE', `/api/mother-closures/${mom.id}`);
  assert.strictEqual(del.status, 200);
  assert.strictEqual(del.data.restored, false);
...
test('媽媽護理名單口徑：今日應入住（reserved）的媽媽也列入，護理端點可用', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  const base = Date.now() - new Date().getTimezoneOffset() * 60000;
  const D = n => new Date(base + n * 86400000).toISOString().slice(0, 10);
  const mom = (await req('POST', '/api/mothers', { name: '今日應入住護理媽', phone: '0900242424' })).data;
  const rooms = (await req('GET', '/api/rooms')).data.filter(r => r.active && r.room_type !== '托嬰');
  let ok = false, room = null;
  for (const r of rooms) {
    // 今日入住、遠期退房 → 房況呈現 due_in（尚未辦入住，狀態仍 reserved）
    const t = await req('POST', '/api/bookings', { mother_id: mom.id, room_id: r.id, check_in: D(0), check_out: D(20), deposit: 0, total_amount: 100000 });
    if (t.status === 200) { ok = true; room = r; break; }
  }
  assert.ok(ok, '需可建今日入住訂房');
  // 媽媽仍為 reserved（未辦入住），但房況應以 due_in／next_booking 帶出她 → 護理名單即依此
  const mm = (await req('GET', '/api/mothers')).data.find(m => m.id === mom.id);
  assert.strictEqual(mm.status, 'reserved');
  const rr = (await req('GET', '/api/room-status/mothers')).data.rooms.find(x => x.id === room.id);
  assert.strictEqual(rr.state, 'due_in', '今日應入住房態應為 due_in');
  assert.ok(rr.next_booking && rr.next_booking.mother_id === mom.id, '應以 next_booking 帶出今日應入住媽媽');
  // 護理端點對 reserved（今日應入住）媽媽可用（護理名單點入即該客戶）
  assert.strictEqual((await req('GET', `/api/mothers/${mom.id}/nursing`)).status, 200);
  assert.strictEqual((await req('GET', `/api/mothers/${mom.id}/closure`)).status, 200);
  // 清理
  const bkId = (await req('GET', `/api/customers/${mom.id}`)).data.bookings[0].id;
  await req('PUT', `/api/bookings/${bkId}/status`, { status: 'cancelled' });
});
...
test('轉房後照護紀錄房號依當時住的房顯示，房況帶出轉房歷程', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  const base = Date.now() - new Date().getTimezoneOffset() * 60000;
  const D = n => new Date(base + n * 86400000).toISOString().slice(0, 10);
  const mom = (await req('POST', '/api/mothers', { name: '轉房紀錄媽', phone: '0900321321' })).data;
  // 入住中訂房：前 5 天 A 房、第 5 天轉 B 房
  const rooms = (await req('GET', '/api/rooms')).data.filter(r => r.active && r.room_type !== '托嬰');
  let bk = null, roomA = null;
  for (const r of rooms) {
    const t = await req('POST', '/api/bookings', { mother_id: mom.id, room_id: r.id,
      check_in: D(-5), check_out: D(5), deposit: 0, total_amount: 100000 });
    if (t.status === 200) { bk = t.data; roomA = r; break; }
  }
  assert.ok(bk, '需可建訂房');
  assert.strictEqual((await req('PUT', `/api/bookings/${bk.id}/status`, { status: 'checked_in' })).status, 200);
  // 轉房前記一筆照護紀錄（應對應 A 房）
  assert.strictEqual((await req('POST', `/api/mothers/${mom.id}/records`, { record_type: 'note', value_text: '轉房前紀錄', recorded_at: `${D(-3)} 09:00` })).status, 200);
  // 今日轉房到另一間
  const roomB = rooms.find(r => r.id !== roomA.id);
  const tr = await req('POST', `/api/bookings/${bk.id}/transfer`,
    { transfer_date: D(0), room_id: roomB.id, reason: '測試轉房' });
  assert.strictEqual(tr.status, 200, `轉房應成功：${JSON.stringify(tr.data)}`);
  // 轉房後記一筆（現在時間＝轉房日當日，應對應 B 房）
  assert.strictEqual((await req('POST', `/api/mothers/${mom.id}/records`,
    { record_type: 'note', value_text: '轉房後紀錄' })).status, 200);
  // 照護紀錄查詢：兩筆房號應不同，且各自對應當時的房
  const recs = (await req('GET', `/api/care-records/query?kind=mother&kw=${encodeURIComponent('轉房紀錄媽')}`)).data;
  const before = recs.find(r => r.detail === '轉房前紀錄');
  const after = recs.find(r => r.detail === '轉房後紀錄');
  assert.ok(before && after, '兩筆紀錄都應查得到');
  assert.strictEqual(before.room_name, roomA.name, '轉房前紀錄應顯示原房號');
  assert.strictEqual(after.room_name, roomB.name, '轉房後紀錄應顯示新房號');
  assert.notStrictEqual(before.room_name, after.room_name, '不可被最後一段整批覆蓋');
  // 房況：轉房歷程（轉房 log）
  const occ = (await req('GET', '/api/room-status/mothers')).data.rooms
    .map(r => r.occupant).find(o => o && o.mother_id === mom.id);
  assert.ok(occ, '應有在住卡片');
  assert.strictEqual(occ.transfers, 1, '應記錄轉房 1 次');
  assert.strictEqual((occ.segments || []).length, 2, '應有兩段房段');
  assert.deepStrictEqual(occ.segments.map(s => s.room_name), [roomA.name, roomB.name], '房段順序應為 A→B');
  // 清理：結束此住期，避免這位在住媽媽被後續共用 fixture 取用（轉房會留下請備房任務）
  for (const s of occ.segments) await req('PUT', `/api/bookings/${s.id}/status`, { status: 'checked_out' });
});

test('請備房任務同名不重複建立，但轉房與報喜的說明會合併進備註', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  const base = Date.now() - new Date().getTimezoneOffset() * 60000;
  const D = n => new Date(base + n * 86400000).toISOString().slice(0, 10);
  const TODAY = D(0);
  const c = await req('POST', '/api/customers', { name: '備房備註合併媽', due_date: TODAY });
  const momId = c.data.id;
  const rooms = (await req('GET', '/api/rooms')).data.filter(r => r.active && r.room_type !== '托嬰');
  let bk = null, roomA = null;
  for (const r of rooms) {
    const t = await req('POST', '/api/bookings', { mother_id: momId, room_id: r.id,
      check_in: D(-4), check_out: D(6), deposit: 0, total_amount: 100000 });
    if (t.status === 200) { bk = t.data; roomA = r; break; }
  }
  assert.ok(bk, '需可建訂房');
  assert.strictEqual((await req('PUT', `/api/bookings/${bk.id}/status`, { status: 'checked_in' })).status, 200);
  // 今日轉房 → 先建立「(新房)請備房MMdd」任務（備註為轉房說明）
  const roomB = rooms.find(r => r.id !== roomA.id);
  assert.strictEqual((await req('POST', `/api/bookings/${bk.id}/transfer`,
    { transfer_date: TODAY, room_id: roomB.id, reason: '合併測試' })).status, 200);
  const taskName = `${roomB.name}請備房${TODAY.slice(5).replace('-', '')}`;
  const hk1 = (await req('GET', `/api/housekeeping?date=${TODAY}`)).data.tasks
    .filter(t => t.task === taskName && t.mother_name === '備房備註合併媽');
  assert.strictEqual(hk1.length, 1, '轉房應建立一筆請備房任務');
  assert.ok(hk1[0].note.includes('轉房'), '備註應含轉房說明');
  // 同日寶寶報喜（入住段房號＝轉房後的房、入住日＝今日）→ 不重複建任務，但併入哺乳衣等資訊
  const ann = await req('POST', `/api/customers/${momId}/baby-announce`, {
    birth_date: TODAY, birth_mode: '自然產', birth_hospital: '禾馨',
    babies: [{ gender: 'female', weight_g: 3100 }],
    meal_choice: 'A', diet_type: '葷食', bra_size: 'L' });
  assert.strictEqual(ann.status, 200);
  const hk2 = (await req('GET', `/api/housekeeping?date=${TODAY}`)).data.tasks
    .filter(t => t.task === ann.data.hk_task && t.mother_name === '備房備註合併媽');
  assert.strictEqual(hk2.length, 1, '不應重複建立同名任務');
  if (ann.data.hk_task === taskName) {
    assert.ok(hk2[0].note.includes('哺乳衣 L'), '報喜資訊（哺乳衣）應併入既有任務備註');
    assert.ok(hk2[0].note.includes('轉房'), '原轉房說明應保留');
  } else {
    assert.ok(hk2[0].note.includes('哺乳衣 L'), '報喜任務備註應含哺乳衣');
  }
  // 清理
  const segs = (await req('GET', `/api/customers/${momId}`)).data.bookings.filter(b => b.status !== 'cancelled');
  for (const s of segs) await req('PUT', `/api/bookings/${s.id}/status`, { status: 'checked_out' });
});

test('縮短住期連動托嬰段：托嬰整段順移到新出住日，天數不變', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  const base = Date.now() - new Date().getTimezoneOffset() * 60000;
  const D = n => new Date(base + n * 86400000).toISOString().slice(0, 10);
  const mom = (await req('POST', '/api/mothers', { name: '縮短連動托嬰媽', phone: '0900654654' })).data;
  const rooms = (await req('GET', '/api/rooms')).data.filter(r => r.active && r.room_type !== '托嬰');
  let bk = null;
  for (const r of rooms) {
    const t = await req('POST', '/api/bookings', { mother_id: mom.id, room_id: r.id,
      check_in: D(-2), check_out: D(8), deposit: 0, total_amount: 100000 });   // 住 10 天
    if (t.status === 200) { bk = t.data; break; }
  }
  assert.ok(bk, '需可建訂房');
  assert.strictEqual((await req('PUT', `/api/bookings/${bk.id}/status`, { status: 'checked_in' })).status, 200);
  // 出住後續托 5 天（D(8)~D(13)）
  const dcRoom = (await req('GET', '/api/rooms?include_daycare=1')).data.find(r => r.room_type === '托嬰');
  assert.ok(dcRoom, '需有托嬰室');
  const dc = await req('POST', '/api/bookings', { mother_id: mom.id, room_id: dcRoom.id,
    check_in: D(8), check_out: D(13), deposit: 0, total_amount: 0 });
  assert.strictEqual(dc.status, 200);
  // 第 N 天提早出住 → 托嬰應改為新出住日起算 5 天
  const leave = D(3);
  const sh = await req('POST', `/api/bookings/${bk.id}/shorten`, { leave_date: leave, reason: '提早返家' });
  assert.strictEqual(sh.status, 200);
  assert.ok(sh.data.daycare_moved && sh.data.daycare_moved.ok, '應回報托嬰段已順移');
  assert.strictEqual(sh.data.daycare_moved.to, leave);
  assert.strictEqual(sh.data.daycare_moved.days, 5, '托嬰天數應不變');
  const dcAfter = (await req('GET', `/api/customers/${mom.id}`)).data.bookings.find(x => x.id === dc.data.id);
  assert.strictEqual(dcAfter.check_in, leave, '托嬰起始日應為新出住日');
  assert.strictEqual(dcAfter.check_out, D(8), '托嬰結束日應為新出住日＋5 天');
  // 清理：釋放托嬰室與房間，避免佔用期間影響後續測試
  await req('PUT', `/api/bookings/${dc.data.id}/status`, { status: 'cancelled' });
  await req('POST', `/api/bookings/${bk.id}/checkout-complete`, {});
});

test('入住前改房型後重簽：合約內容依目前房型重新產生（不再沿用舊房型快照）', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  const base = Date.now() - new Date().getTimezoneOffset() * 60000;
  const D = n => new Date(base + n * 86400000).toISOString().slice(0, 10);
  const IN = D(120);   // 遠期，避開既有訂房
  const mom = (await req('POST', '/api/mothers', { name: '改房型重簽媽', phone: '0900456456' })).data;
  await req('PUT', `/api/customers/${mom.id}/contract`, { handler: '王主任', expected_check_in: IN });
  assert.strictEqual((await req('POST', `/api/customers/${mom.id}/contract/items`, { name: '豪華房', qty: 10 })).status, 200);
  // 排一段豪華房
  const deluxe = (await req('GET', '/api/rooms')).data.find(r => r.active && r.room_type === '豪華房');
  assert.ok(deluxe, '需有豪華房');
  const bk = (await req('POST', '/api/bookings', { mother_id: mom.id, room_id: deluxe.id,
  ...
  // 改房型 豪華房 → 標準房（自動改排床）
  const ch = await req('POST', `/api/customers/${mom.id}/contract/items/change-type`, { index: 0, name: '標準房' });
  assert.strictEqual(ch.status, 200);
  // 重新產生內容：應為標準房，且不再含豪華房
  const re = await req('GET', `/api/contracts/${ec.id}/rerender`);
  assert.strictEqual(re.status, 200);
  assert.strictEqual(re.data.room_type, '標準房', '重新產生應取目前房型');
  assert.ok(re.data.body.includes('標準房'), '重新產生的內容應含標準房');
  assert.ok(!re.data.body.includes('豪華房'), '重新產生的內容不應殘留豪華房');
  // 後端防線：重簽未帶內容時，已標示需重簽者也應依目前資料重新產生（不得沿用舊房型）
  assert.ok((await req('GET', '/api/contracts')).data.find(x => x.id === ec.id).needs_resign,
    '改房型後原合約應標示需重簽');
  const rs = await req('POST', `/api/contracts/${ec.id}/resign`, { title: c1.title });
  assert.strictEqual(rs.status, 200);
  const c2 = (await req('GET', `/api/contracts/${rs.data.id}`)).data;
  assert.ok(c2.body.includes('標準房'), '重簽後新版應為標準房');
  assert.ok(!c2.body.includes('豪華房'), '重簽後新版不應殘留豪華房');
  assert.strictEqual((await req('GET', `/api/contracts/${ec.id}`)).data.status, 'void', '原合約應作廢');
});

test('潛在客戶重複偵測：同姓名＋同預產期回 409 候選，force 可仍新增；電話比對亦成立', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  const c1 = await req('POST', '/api/customers', { name: '重複偵測媽', due_date: '2026-09-09', phone: '0911222333' });
  assert.strictEqual(c1.status, 200);
  // 同姓名＋同預產期 → 409，附候選清單
  const dup = await req('POST', '/api/customers', { name: '重複偵測媽', due_date: '2026-09-09' });
  assert.strictEqual(dup.status, 409);
  assert.ok(Array.isArray(dup.data.duplicates) && dup.data.duplicates.some(x => x.id === c1.data.id));
  // 帶 force 仍可新增（確定是不同人）
  const forced = await req('POST', '/api/customers', { name: '重複偵測媽', due_date: '2026-09-09', force: true });
  assert.strictEqual(forced.status, 200);
  assert.notStrictEqual(forced.data.id, c1.data.id);
  // 電話相同也算重複（即使姓名/預產期不同）
  const dupPhone = await req('POST', '/api/customers', { name: '不同名', due_date: '2027-01-01', phone: '0911222333' });
  assert.strictEqual(dupPhone.status, 409);
});

test('刪除潛在客戶：空白潛客可刪；有排房者擋下', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  // 空白潛客 → 可刪
  const empty = await req('POST', '/api/customers', { name: '待刪空白潛客', due_date: '2026-10-10' });
  assert.strictEqual((await req('DELETE', `/api/customers/${empty.data.id}`)).status, 200);
  assert.ok(!(await req('GET', '/api/mothers')).data.some(m => m.id === empty.data.id), '刪除後不應再出現');
  // 有排房 → 擋下
  const withBk = await req('POST', '/api/customers', { name: '有排房潛客', due_date: '2026-10-11' });
  const rooms = (await req('GET', '/api/rooms')).data.filter(r => r.active && r.room_type !== '托嬰');
  let made = false;
  for (const r of rooms) {
    const t = await req('POST', '/api/bookings', { mother_id: withBk.data.id, room_id: r.id, check_in: '2026-10-11', check_out: '2026-10-20' });
    if (t.status === 200) { made = true; break; }
  }
  assert.ok(made, '需可建訂房');
  const blocked = await req('DELETE', `/api/customers/${withBk.data.id}`);
  assert.strictEqual(blocked.status, 409);
  assert.ok(blocked.data.error.includes('排房'));
  // 清理：取消該筆排房後即可刪除，避免佔用房間影響後續測試
  const bkId = (await req('GET', `/api/customers/${withBk.data.id}`)).data.bookings[0].id;
  assert.strictEqual((await req('PUT', `/api/bookings/${bkId}/status`, { status: 'cancelled' })).status, 200);
  assert.strictEqual((await req('DELETE', `/api/customers/${withBk.data.id}`)).status, 200);
});
...
test('三者互不連動：退房完成／產婦結案／產後嬰兒結案各自獨立，未結案者留原房號', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  const TODAY = new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 10);
  const c = await req('POST', '/api/customers', { name: '三者獨立媽', due_date: TODAY });
  const momId = c.data.id;
  const rooms = (await req('GET', '/api/rooms')).data.filter(r => r.active).reverse();
  let bk = null, room = null;
  for (const r of rooms) {
    const t = await req('POST', '/api/bookings', { mother_id: momId, room_id: r.id, check_in: '2026-01-01', check_out: TODAY });
    if (t.status === 200) { bk = t.data; room = r; break; }
  }
  assert.ok(bk, '需可建訂房');
  assert.strictEqual((await req('PUT', `/api/bookings/${bk.id}/status`, { status: 'checked_in' })).status, 200);
  const baby = await req('POST', '/api/babies', { mother_id: momId, name: '三者獨立寶', gender: 'female', birth_date: TODAY });
  assert.strictEqual(baby.status, 200);
  const babyId = baby.data.id;

  // (1) 產婦結案：不建清潔任務、不退房、房況與寶寶房況都不受影響
  assert.strictEqual((await req('PUT', `/api/mothers/${momId}/closure`, {
    close_date: TODAY, close_time: '10:00', reason: '期滿結案', destination: '返家' })).status, 200);
  const hk1 = (await req('GET', `/api/housekeeping?date=${TODAY}`)).data;
  assert.ok(!hk1.tasks.some(t => t.task === `${room.name}房已出住` && t.note === '產婦結案自動建立'),
    '產婦結案不應再自動建清潔任務（該由退房完成負責）');
  const mAfterClose = (await req('GET', '/api/mothers')).data.find(m => m.id === momId);
  assert.strictEqual(mAfterClose.status, 'checked_in', '產婦結案不得連動退房');
  const babiesAfterClose = (await req('GET', '/api/room-status/babies')).data.babies;
  assert.ok(babiesAfterClose.some(b => b.id === babyId), '產婦結案不得讓寶寶從寶寶房況消失');

  // (2) 退房完成：唯一改訂房狀態者；建清潔任務；寶寶未結案故仍留在寶寶房況
  const done = await req('POST', `/api/bookings/${bk.id}/checkout-complete`, {});
  assert.strictEqual(done.status, 200);
  const hk2 = (await req('GET', `/api/housekeeping?date=${TODAY}`)).data;
  assert.ok(hk2.tasks.some(t => t.task === `${room.name}房已出住` && t.note === '退房完成自動建立'),
    '退房完成應建立清潔任務');
  assert.strictEqual((await req('GET', '/api/mothers')).data.find(m => m.id === momId).status, 'checked_out');
  const babiesAfterOut = (await req('GET', '/api/room-status/babies')).data.babies;
  const stillBaby = babiesAfterOut.find(b => b.id === babyId);
  assert.ok(stillBaby, '媽媽退房但寶寶未結案，寶寶仍須留在寶寶房況供結案');
  assert.strictEqual(stillBaby.pending_closure, 1, '應標示已退房・待結案');
  assert.ok(stillBaby.room_name, '待結案寶寶仍應帶出原房號');

  // (3) 產後嬰兒結案：獨立完成後才從待結案名單移除
  assert.strictEqual((await req('PUT', `/api/babies/${babyId}/closure`, {
    close_date: TODAY, close_time: '11:00', reason: '期滿結案', destination: '返家' })).status, 200);
  const babiesFinal = (await req('GET', '/api/room-status/babies')).data.babies;
  assert.ok(!babiesFinal.some(b => b.id === babyId && b.pending_closure),
    '嬰兒結案後不應再列為待結案');
});
...
test('已退房未辦產婦結案者：媽媽房況留在原房號（pending_closures）供結案', async () => {
  ...
  assert.ok((rr.pending_closures || []).some(p => p.mother_id === momId), '應列入該房的待結案名單');
  assert.ok(rs.stats.pending_closure >= 1, '統計應含待結案筆數');
  // 辦完產婦結案後移出待結案名單
  assert.strictEqual((await req('PUT', `/api/mothers/${momId}/closure`, {
    close_date: TODAY, close_time: '10:00', reason: '期滿結案', destination: '返家' })).status, 200);
...
test('膳食：出住日當天仍保留該房供餐（媽媽出現在出住日的膳食名單）', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  const mom = await anyCheckedInMother();
  const bk = (await req('GET', `/api/customers/${mom.id}`)).data.bookings.find(b => b.status === 'checked_in');
  assert.ok(bk, '需有入住中訂房');
  const outList = (await req('GET', `/api/meals?date=${bk.check_out}`)).data.mothers;
  const me = outList.find(x => x.id === mom.id);
  assert.ok(me, '出住日當天膳食名單仍應包含該媽媽（供早餐）');
  assert.strictEqual(me.checkout_today, true, '應標示為當日出住');
  // 出住日之後隔天，該媽媽不應再出現在膳食名單
  const after = new Date(new Date(bk.check_out).getTime() + 86400000).toISOString().slice(0, 10);
  const afterList = (await req('GET', `/api/meals?date=${after}`)).data.mothers;
  assert.ok(!afterList.find(x => x.id === mom.id), '出住日隔天不應再供餐');
});

test('房況表：未到期預約不佔房（不顯示為下一筆／今日入住），僅當日入住才帶入', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  // 建一位媽媽＋遠期預約（10 天後入住），該預約不得出現在任何房的 next_booking
  const mom = (await req('POST', '/api/mothers', { name: '遠期預約媽', phone: '0900123321' })).data;
  const room = (await req('GET', '/api/rooms')).data.find(r => r.active && r.room_type !== '托嬰');
  const inD = new Date(Date.now() + 10 * 86400000).toISOString().slice(0, 10);
  const outD = new Date(Date.now() + 40 * 86400000).toISOString().slice(0, 10);
  const bk = await req('POST', '/api/bookings', { mother_id: mom.id, room_id: room.id, check_in: inD, check_out: outD, deposit: 0, total_amount: 1000 });
  assert.strictEqual(bk.status, 200);
  const rooms = (await req('GET', '/api/room-status/mothers')).data.rooms;
  const hit = rooms.find(r => r.next_booking && r.next_booking.mother_id === mom.id);
  assert.ok(!hit, '未到期（10 天後）的預約不應顯示為房況的下一筆／今日入住');
});
...
test('托嬰：房型/托嬰室已建、排托嬰後媽媽退房寶寶仍在房況且轉為托嬰、結案日帶托嬰結束日', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  // 1) 房型設定有「托嬰」，且有一間托嬰室 → 銷售房型/安排房型選單自動出現
  const types = (await req('GET', '/api/room-types')).data;
  assert.ok(types.some(t => t.name === '托嬰'), '房型應含托嬰');
  // 一般選房不應出現托嬰室；帶 include_daycare=1 才可見（排房「安排房型＝托嬰」用）
  const plain = (await req("GET", "/api/rooms")).data;
  assert.ok(!plain.some(r => r.room_type === "托嬰"), "一般選房清單不應含托嬰室");
  const rooms = (await req("GET", "/api/rooms?include_daycare=1")).data;
  const dcRoom = rooms.find(r => r.room_type === "托嬰");
  assert.ok(dcRoom, "應有托嬰室");
  // 床表：一般床表排除托嬰；排房情境（include_daycare=1）需顯示托嬰房才能排托嬰段／排床
  const cal0 = (await req('GET', '/api/room-calendar?start=2027-03-01&days=30')).data;
  assert.ok(!cal0.rooms.some(r => r.room_type === '托嬰'), '一般床表不應含托嬰房');
  const calDc = (await req('GET', '/api/room-calendar?start=2027-03-01&days=30&include_daycare=1')).data;
  assert.ok(calDc.rooms.some(r => r.room_type === '托嬰'), '排房床表應顯示托嬰房');
  // 2) 媽媽房況不應出現托嬰室
  const mb = (await req('GET', '/api/room-status/mothers')).data;
  assert.ok(!mb.rooms.some(r => r.room_type === '托嬰'), '媽媽房況不應含托嬰室');
  // 3) 建一筆涵蓋今日的托嬰排房，並讓媽媽退房
  const mom = await anyCheckedInMother();
  const baby = (await req('GET', '/api/babies')).data.find(b => b.mother_id === mom.id);
  assert.ok(baby, '需有該媽媽的寶寶');
  const today = new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 10);
  const end = new Date(Date.now() + 10 * 86400000 - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 10);
  const bkNew = await req('POST', '/api/bookings', {
    mother_id: mom.id, room_id: dcRoom.id, check_in: today, check_out: end,
    deposit: 0, total_amount: 0, notes: '托嬰測試'
  });
  assert.strictEqual(bkNew.status, 200);
  // 媽媽退房
  const cust = (await req('GET', `/api/customers/${mom.id}`)).data;
  const inBk = cust.bookings.find(b => b.status === 'checked_in' && b.room_id !== dcRoom.id);
  if (inBk) await req('PUT', `/api/bookings/${inBk.id}/status`, { status: 'checked_out' });
  // 4) 寶寶房況：媽媽已退房但寶寶仍在，且房號顯示托嬰、狀態轉為 daycare
  const board = (await req('GET', '/api/room-status/babies')).data;
  const bb = board.babies.find(x => x.id === baby.id);
  assert.ok(bb, '托嬰期間寶寶應仍出現在寶寶房況');
  assert.strictEqual(bb.room_name, '托嬰', '房號應顯示托嬰');
  assert.strictEqual(bb.location, 'daycare', '狀態應自動轉為托嬰');
  assert.strictEqual(bb.daycare_end, end, '應帶出托嬰結束日');
  assert.ok(board.stats.daycare >= 1, '統計應有托嬰人數');
});

test('排房變更連動收費：PUT 訂房帶 total_amount 後帳務應收同步', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  // 自建訂房（遠期空窗），避免共用 fixture 被其他測試佔用而撞期
  const base = Date.now() - new Date().getTimezoneOffset() * 60000;
  const D = n => new Date(base + n * 86400000).toISOString().slice(0, 10);
  const mom = (await req('POST', '/api/mothers', { name: '排房收費同步媽', phone: '0900888999' })).data;
  const rooms = (await req('GET', '/api/rooms')).data.filter(r => r.active && r.room_type !== '托嬰');
  let bk = null;
  for (const r of rooms) {
    const t = await req('POST', '/api/bookings', { mother_id: mom.id, room_id: r.id,
      check_in: D(200), check_out: D(210), deposit: 0, total_amount: 10 * 6800 });
    if (t.status === 200) { bk = t.data; break; }
  }
  assert.ok(bk, '需可建訂房');
  const newOut = D(220), newTotal = 20 * 6800;
  const r = await req('PUT', `/api/bookings/${bk.id}`, { check_in: D(200), check_out: newOut, total_amount: newTotal });
  assert.strictEqual(r.status, 200);
  const bill = (await req('GET', '/api/billing')).data.find(x => x.id === bk.id);
  assert.strictEqual(bill.total_amount, newTotal, '收費帳務應收應同步為新天數×單價');
  assert.strictEqual(bill.check_out, newOut);
  await req('PUT', `/api/bookings/${bk.id}/status`, { status: 'cancelled' });   // 清理，釋放房間
});

test('入住中縮短住期：退房日/應收改定並連動訂餐，非入住中或日期不合被擋', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  // 自建入住中訂房（前面測試可能已把示範媽媽退房）
  const TODAY = new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 10);
  const OUT = new Date(Date.now() - new Date().getTimezoneOffset() * 60000 + 8 * 86400000).toISOString().slice(0, 10);
  const mom = (await req('POST', '/api/mothers', { name: '縮短住期媽', phone: '0900777888' })).data;
  const roomsFree = (await req('GET', '/api/rooms')).data.filter(r => !r.occupant);
  let bk = null;
  for (const r of roomsFree) {
    const t = await req('POST', '/api/bookings', { mother_id: mom.id, room_id: r.id, check_in: TODAY, check_out: OUT, deposit: 0, total_amount: 8 * 6800 });
    if (t.status === 200) { bk = { id: t.data.id, mother_id: mom.id, check_in: TODAY, check_out: OUT }; break; }
  }
  assert.ok(bk, '找不到可訂房間');
  assert.strictEqual((await req('PUT', `/api/bookings/${bk.id}/status`, { status: 'checked_in' })).status, 200);
  // 未入住中不可縮短已由下方 400 分支涵蓋；先驗證日期檢核
  // 日期不合被擋
  assert.strictEqual((await req('POST', `/api/bookings/${bk.id}/shorten`, { leave_date: bk.check_out })).status, 400);
  assert.strictEqual((await req('POST', `/api/bookings/${bk.id}/shorten`, {})).status, 400);
  // 縮短 2 天
  const leave = new Date(new Date(bk.check_out).getTime() - 2 * 86400000).toISOString().slice(0, 10);
  const r = await req('POST', `/api/bookings/${bk.id}/shorten`, { leave_date: leave, reason: '提前返家' });
  assert.strictEqual(r.status, 200);
  assert.strictEqual(r.data.check_out, leave);
  assert.strictEqual(r.data.total_amount, r.data.quote.used_fee + r.data.quote.penalty + r.data.quote.handling);
  // 帳務同步
  const bill = (await req('GET', '/api/billing')).data.find(x => x.id === bk.id);
  assert.strictEqual(bill.check_out, leave);
  assert.strictEqual(bill.total_amount, r.data.total_amount);
  // 訂餐連動：新退房日之後不應再有訂餐、新退房日僅早餐
  const after = new Date(new Date(leave).getTime() + 86400000).toISOString().slice(0, 10);
  const oAfter = (await req('GET', `/api/meals?date=${after}`)).data.orders.filter(o => o.mother_id === bk.mother_id);
  assert.strictEqual(oAfter.length, 0, '新退房日之後不應有訂餐');
  const oLeave = (await req('GET', `/api/meals?date=${leave}`)).data.orders.filter(o => o.mother_id === bk.mother_id);
  assert.ok(!oLeave.some(o => o.meal_type !== 'breakfast'), '新退房日僅供早餐');
  // 已退房訂房不可再縮短
  // （此訂房仍 checked_in，改測預約中：略——狀態限制已由 400 分支涵蓋）
});

test('入住前合約明細變更連動：改天數/刪除同步排房與訂餐、標示需重簽、入住後鎖定', async () => {
  await req('POST', '/api/login', { username: 'admin', password: 'admin123' });
  const TODAY = new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 10);
  const addD = (d, n) => new Date(new Date(d + 'T00:00:00Z').getTime() + n * 86400000).toISOString().slice(0, 10);
  const IN = addD(TODAY, 60);   // 遠期，避開既有訂房
  const mom = (await req('POST', '/api/mothers', { name: '入住前調整媽', phone: '0900123123' })).data;
  // 建合約＋兩筆明細（20天＋10天）
  await req('PUT', `/api/customers/${mom.id}/contract`, { handler: '王主任', expected_check_in: IN });
  assert.strictEqual((await req('POST', `/api/customers/${mom.id}/contract/items`, { name: '標準房', qty: 20 })).status, 200);
  assert.strictEqual((await req('POST', `/api/customers/${mom.id}/contract/items`, { name: '標準房', qty: 10 })).status, 200);
  // 排兩段（同一房接續）
  const room = (await req('GET', '/api/rooms')).data.find(r => r.room_type === '標準房');
  const bk1 = (await req('POST', '/api/bookings', { mother_id: mom.id, room_id: room.id, check_in: IN, check_out: addD(IN, 20), deposit: 0, total_amount: 20 * 6800 })).data;
  const bk2 = (await req('POST', '/api/bookings', { mother_id: mom.id, room_id: room.id, check_in: addD(IN, 20), check_out: addD(IN, 30), deposit: 0, total_amount: 10 * 6800 })).data;
  assert.ok(bk1.id && bk2.id);
  // 種一筆訂餐（模擬報喜鋪餐），驗證後續重鋪
  ...
  assert.strictEqual(e1.data.sync.updated, 2);
  const bks1 = (await req('GET', `/api/customers/${mom.id}`)).data.bookings.filter(b => b.status === 'reserved').sort((a, b) => a.check_in < b.check_in ? -1 : 1);
  assert.strictEqual(bks1[0].check_out, addD(IN, 15));
  assert.strictEqual(bks1[0].total_amount, 15 * 6800);
  assert.strictEqual(bks1[1].check_in, addD(IN, 15));
  assert.strictEqual(bks1[1].check_out, addD(IN, 25));
  // 訂餐重鋪：新總住期最後一天僅早餐、之後無
  const lastDay = addD(IN, 25);
  const oLast = (await req('GET', `/api/meals?date=${lastDay}`)).data.orders.filter(o => o.mother_id === mom.id);
  assert.ok(oLast.length === 1 && oLast[0].meal_type === 'breakfast', '出住日僅早餐');
  assert.strictEqual((await req('GET', `/api/meals?date=${addD(IN, 26)}`)).data.orders.filter(o => o.mother_id === mom.id).length, 0);
  // 電子合約被標示需重簽
  const ecRow = (await req('GET', '/api/contracts')).data.find(c => c.id === ec.id);
  assert.strictEqual(ecRow.needs_resign, 1, '已建合約應標示需重簽');
  // 2) 刪第二筆明細：對應預約段連動取消
  const del = await req('POST', `/api/customers/${mom.id}/contract/items/delete`, { index: 1, reason: '縮短住期' });
  assert.strictEqual(del.status, 200);
  assert.strictEqual(del.data.sync.cancelled, 1);
  const bks2 = (await req('GET', `/api/customers/${mom.id}`)).data.bookings;
  assert.strictEqual(bks2.filter(b => b.status === 'reserved').length, 1);
  assert.strictEqual(bks2.filter(b => b.status === 'cancelled').length, 1);
  // 3) 入住後鎖定
  const keep = bks2.find(b => b.status === 'reserved');
  await req('PUT', `/api/bookings/${keep.id}/status`, { status: 'checked_in' });
  assert.strictEqual((await req('POST', `/api/customers/${mom.id}/contract/items`, { name: '標準房', qty: 5 })).status, 400);
  assert.strictEqual((await req('POST', `/api/customers/${mom.id}/contract/items/edit`, { index: 0, qty: 5 })).status, 400);
  assert.strictEqual((await req('POST', `/api/customers/${mom.id}/contract/items/delete`, { index: 0, reason: 'x' })).status, 400);
  // 還原退房，避免影響其他測試
