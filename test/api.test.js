...
test('寶寶報喜：自動建請備房任務＋入住日訂餐；7日內入住帶完整欄位', async () => {
  ...
  assert.strictEqual(ann.data.hk_task, `${room.name}請備房`);
  assert.strictEqual(ann.data.meal_date, IN);
  // 房務任務：入住日排定、備註含媽咪／房號／入出住日／哺乳衣
  const hk = (await req('GET', `/api/housekeeping?date=${IN}`)).data;
  const task = hk.tasks.find(t => t.task === `${room.name}請備房` && t.mother_name === '通知單測試媽');
  ...
  assert.strictEqual(hk2.tasks.filter(t => t.task === `${room.name}請備房` && t.mother_name === '通知單測試媽').length, 1);
...
test('訂房改期：報喜帶入的入住日訂餐與請備房任務跟著搬', async () => {
  ...
  assert.strictEqual(newMeals.length, 3);
  assert.ok(newMeals.every(o => o.choice === 'A'));
  // 請備房任務改到 D45，備註日期同步
  const hk = (await req('GET', `/api/housekeeping?date=${D(45)}`)).data;
  const task = hk.tasks.find(t => t.task === `${room.name}請備房` && t.mother_name === '改期同步媽');
  assert.ok(task);
  assert.strictEqual(task.scheduled_for, D(45));
