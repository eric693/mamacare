...
const MODULES = [
  ...
  { key: 'proc_request', label: '採購－請購人員' },
  { key: 'proc_buyer', label: '採購－採購人員' },
  { key: 'proc_receive', label: '採購－驗貨人員' },
  { key: 'proc_ship', label: '採購－出貨人員' },
  { key: 'proc_account', label: '採購－記帳人員' },
  { key: 'proc_finance', label: '採購－財務人員（全部可瀏覽）' },
  { key: 'proc_admin', label: '採購－採購作業管理員' },
...
const MODULE_RULES = [
  ...
  // 採購作業：七種角色任一即可進入，各頁瀏覽／key 單由 routes/procurement.js 依角色再擋
  [/^\/api\/procurement/, ['proc_request', 'proc_buyer', 'proc_receive', 'proc_ship', 'proc_account', 'proc_finance', 'proc_admin']],
...
// 評量結果依紙本填表說明以符號呈現
const MGS_RESULTS = ['O', '△', 'X'];
const MGS_RESULT_LABELS = {
  O: 'O：完全可以說出且可回覆示教',
  '△': '△：能部分說出，部分可回覆示教',
  X: 'X：完全不能說出，無法執行'
};
...
app.get('/api/mothers/:id/guidance-sheet', requireStaff, (req, res) => {
  ...
  res.json({ mother, items, entries: data.entries || {}, results: MGS_RESULTS,
    result_labels: MGS_RESULT_LABELS, updated_at: row ? row.updated_at : '' });
});
...
// ---------- 寶寶護理指導單（新生兒護理衛教指導單；產婦簽名） ----------
app.get('/api/baby-guidance-items', requireStaff, (req, res) => {
  res.json(db.prepare('SELECT id, category, name, options, sort FROM baby_guidance_items WHERE active = 1 ORDER BY sort, id').all());
});

app.put('/api/baby-guidance-items', requireAdmin, (req, res) => {
  const rows = Array.isArray(req.body && req.body.items) ? req.body.items : [];
  const clean = rows.map(r => ({
    id: Number(r.id) || 0,
    category: String(r.category || '').trim().slice(0, 30),
  ...
  const keep = new Set(clean.map(r => r.id).filter(Boolean));
  db.transaction(() => {
    for (const r of db.prepare('SELECT id FROM baby_guidance_items WHERE active = 1').all()) {
      if (!keep.has(r.id)) db.prepare('UPDATE baby_guidance_items SET active = 0 WHERE id = ?').run(r.id);
    }
    clean.forEach((r, i) => {
      if (r.id && db.prepare('SELECT 1 FROM baby_guidance_items WHERE id = ?').get(r.id)) {
        db.prepare('UPDATE baby_guidance_items SET category=?, name=?, options=?, sort=?, active=1 WHERE id=?')
          .run(r.category, r.name, r.options, (i + 1) * 10, r.id);
      } else {
        db.prepare('INSERT INTO baby_guidance_items (category, name, options, sort) VALUES (?,?,?,?)')
          .run(r.category, r.name, r.options, (i + 1) * 10);
      }
    });
  })();
  logAudit(req, { action: 'update', entity: 'baby_guidance_items', entity_id: 0, summary: `寶寶護理指導評量項目維護（${clean.length} 項）` });
  res.json({ ok: true });
});

app.get('/api/babies/:id/guidance-sheet', requireStaff, (req, res) => {
  const baby = db.prepare(`
    SELECT b.*, m.name AS mother_name,
      (SELECT r.name FROM bookings bk JOIN rooms r ON r.id = bk.room_id
        WHERE bk.mother_id = m.id AND bk.status IN ('checked_in','reserved')
    ...
    FROM babies b JOIN mothers m ON m.id = b.mother_id WHERE b.id = ?`).get(req.params.id);
  if (!baby) return res.status(404).json({ error: '找不到寶寶' });
  const row = db.prepare('SELECT * FROM baby_guidance_sheets WHERE baby_id = ?').get(baby.id);
  let data = {};
  if (row) { try { data = JSON.parse(row.data); } catch (e) { data = {}; } }
  const items = db.prepare('SELECT id, category, name, options FROM baby_guidance_items WHERE active = 1 ORDER BY sort, id').all();
  res.json({ baby, items, entries: data.entries || {}, results: MGS_RESULTS,
    result_labels: MGS_RESULT_LABELS, updated_at: row ? row.updated_at : '' });
});

// 單項存檔（指導簽名／再評量簽名）：日期自動帶入當日，指導者為登入者
app.put('/api/babies/:id/guidance-sheet/:itemId', requireStaff, (req, res) => {
  const baby = db.prepare('SELECT id FROM babies WHERE id = ?').get(req.params.id);
  if (!baby) return res.status(404).json({ error: '找不到寶寶' });
  const item = db.prepare('SELECT id FROM baby_guidance_items WHERE id = ? AND active = 1').get(req.params.itemId);
  if (!item) return res.status(404).json({ error: '找不到評量項目' });
  const b = req.body || {};
  const stage = b.stage === 're' ? 're' : 'guide';
  if (b.result && !MGS_RESULTS.includes(b.result)) return res.status(400).json({ error: '評量結果選項不正確' });
  const row = db.prepare('SELECT * FROM baby_guidance_sheets WHERE baby_id = ?').get(baby.id);
  let data = {};
  if (row) { try { data = JSON.parse(row.data); } catch (e) { data = {}; } }
  ...
  if (json.length > 4000000) return res.status(400).json({ error: '指導單資料過大，請聯絡管理員' });
  if (row) {
    db.prepare(`UPDATE baby_guidance_sheets SET data=?, updated_at=datetime('now','localtime'), updated_by=? WHERE id=?`)
      .run(json, req.session.user.id, row.id);
  } else {
    db.prepare('INSERT INTO baby_guidance_sheets (baby_id, data, updated_by) VALUES (?,?,?)')
      .run(baby.id, json, req.session.user.id);
  }
  logAudit(req, { action: 'update', entity: 'baby_guidance_sheets', entity_id: baby.id,
    summary: `寶寶護理指導單簽名（項目 ${item.id}／${stage === 're' ? '再評量' : '指導'}）` });
  res.json({ ok: true, entry });
});

app.delete('/api/babies/:id/guidance-sheet/:itemId', requireAdmin, (req, res) => {
  const row = db.prepare('SELECT * FROM baby_guidance_sheets WHERE baby_id = ?').get(req.params.id);
  if (!row) return res.json({ ok: true });
  let data = {};
  try { data = JSON.parse(row.data); } catch (e) { data = {}; }
  if (data.entries) delete data.entries[req.params.itemId];
  db.prepare(`UPDATE baby_guidance_sheets SET data=?, updated_at=datetime('now','localtime'), updated_by=? WHERE id=?`)
    .run(JSON.stringify(data), req.session.user.id, row.id);
  logAudit(req, { action: 'delete', entity: 'baby_guidance_sheets', entity_id: Number(req.params.id), summary: '寶寶護理指導單清除單項簽名' });
  res.json({ ok: true });
});
...
app.use('/api', require('./routes/procurement')({ db, requireStaff, logAudit, getSettings, today, buildWorkbook }));
